# =============================================================================
# QUICK ABLATION TEST: GeoSlot 2.0 — ViT Backbone on CVUSA (20%)
# =============================================================================
# PARALLEL SESSION: Run this alongside quick_ablation_cvusa.py (MambaVision)
# Backbone: DINOv2 ViT-B/14 (no mamba_ssm needed -> faster setup!)
#
# 6 Configs (same as MambaVision version):
#   A1: Backbone + GAP  -> Cosine  (Baseline)
#   A2: + Slot Attention
#   A3: + Adaptive BG Mask (gamma learned)
#   A4: + Graph Mamba (Hilbert)      [uses LinearAttention SSM blocks]
#   A5: + FGW OT (balanced)
#   A6: + UFGW (unbalanced)
#
# Each config: 10 epochs, 20% CVUSA -> ~4-6 min per config on H100
# Total: ~25-40 min (faster than Mamba version due to no mamba_ssm build)
# =============================================================================
...
print("[2/2] Setup complete! (No mamba_ssm needed for ViT)")
...
# --- Model (ViT) ---
BACKBONE_NAME  = "vit_base_patch14_dinov2.lvd142m"  # DINOv2 ViT-B/14

FEATURE_DIM    = 768          # ViT-B hidden dim (vs 1568 for MambaVision)
SLOT_DIM       = 256
MAX_SLOTS      = 8
N_REGISTER     = 4
EMBED_DIM_OUT  = 512
...
class ViTBackbone(nn.Module):
    """
    DINOv2 ViT-B/14 as dense feature extractor.
    Handles BOTH satellite (224x224 -> 16x16 patches) and 
    panorama (128x512 -> 9x36 patches) inputs dynamically.
    """
    def __init__(self, model_name="vit_base_patch14_dinov2.lvd142m", frozen=False):
        ...
        if pe and hasattr(pe, 'patch_size'):
            ps = pe.patch_size
            self.ps = ps[0] if isinstance(ps, (tuple, list)) else ps
        else:
            self.ps = 14  # DINOv2 default
    ...
    def forward(self, x):
        ...
        # Compute expected patch grid from ACTUAL input dims
        H_patches = H_in // self.ps   # 224->16, 128->9
        W_patches = W_in // self.ps   # 224->16, 512->36
        expected_n = H_patches * W_patches

        feats = self.model.forward_features(x)  # [B, N_tokens, D]

        if feats.dim() == 3:
            N_tokens = feats.shape[1]
            if N_tokens == expected_n:
                pass  # No CLS token, perfect
            elif N_tokens == expected_n + 1:
                feats = feats[:, 1:, :]  # Remove CLS token
            elif N_tokens > expected_n + 1:
                # CLS + register tokens; remove all, keep last expected_n
                feats = feats[:, 1:, :]  # Remove CLS first
                if feats.shape[1] > expected_n:
                    feats = feats[:, :expected_n, :]  # Keep first expected_n

        return feats, (H_patches, W_patches)


# =============================================================================
# SSM BLOCK (LinearAttention — no mamba_ssm needed)
# =============================================================================
class SSMBlock(nn.Module):
    """LinearAttention SSM block (no mamba_ssm dependency)."""
...
class ConfigA2toA6(nn.Module):
    ...
    def forward(self, q_img, r_img, global_step=None):
        ...
        if ot_out is not None:
            sim = ot_out["similarity"]; tp = ot_out["transport_plan"]; tc = ot_out["transport_cost"]
        else:
            sim = (q["embedding"] * r["embedding"]).sum(dim=-1)
            tp = None; tc = torch.zeros(q_img.shape[0], device=q_img.device)
        return {"query_embedding": q["embedding"], "ref_embedding": r["embedding"],
...
def train_config(config_name, model, train_loader, val_loader, device, epochs=10):
    ...
    global_step = 0; best_r1 = 0.0; history = []
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                optimizer.zero_grad(set_to_none=True); global_step += 1; continue
...
def main():
    ...
    print("  BACKBONE: DINOv2 ViT-B/14 (timm)")
    ...
    # === FINAL RESULTS TABLE ===
    print("\n" + "="*70)
    print("  ABLATION RESULTS SUMMARY (ViT Backbone)")
    print("="*70)
    ...
    results_path = os.path.join(OUTPUT_DIR, "ablation_results_vit.json")
