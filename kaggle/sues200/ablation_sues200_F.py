# =============================================================================
# ABLATION STUDY — SPDGeo-DPEA-MAR (EXP35) — Base Loss Ablations
# =============================================================================
# Full model = EXP35: 12 losses total
#   Base: CE + InfoNCE + PartConsistency + CrossDistill + SelfDistill + UAPA
#   Novel: Proxy + EMA + AltConsist + MaskRecon + AltPred + Diversity
#          + DynamicFusionGate + DeepAltitudeFiLM
#
# Groups E-F ablate the base loss components:
#   E: w/o PartConsistency, w/o CrossDistillation
#   F: w/o SelfDistillation, w/o UAPA
#
# Each ablation runs for 60 epochs. Compare against full EXP35 results.
# =============================================================================

ABLATION_GROUP = "F"  # ← "E" or "F"
ABL_EPOCHS = 60
...
_FULL = {"use_proxy": True, "use_fusion_gate": True, "use_ema": True,
         ...
         "use_part_consist": True, "use_cross_dist": True,
         "use_self_dist": True, "use_uapa": True}

ABLATION_RUNS = {
    "E": [
        {**_FULL, "name": "w/o_PartConsistency", "use_part_consist": False},
        {**_FULL, "name": "w/o_CrossDistill", "use_cross_dist": False},
    ],
    "F": [
        {**_FULL, "name": "w/o_SelfDistill", "use_self_dist": False},
        {**_FULL, "name": "w/o_UAPA", "use_uapa": False},
    ],
}
...
def train_one_epoch(model, teacher, ema, loader, losses, optimizer, scaler,
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # PartConsistency (conditional)
            if abl.get('use_part_consist', True):
                l_con = consist(d_out['parts']['assignment'], s_out['parts']['assignment'])
                loss += CFG.LAMBDA_CONSISTENCY * l_con
                loss_sums['con'] += l_con.item()

            # CrossDistillation (conditional)
            if abl.get('use_cross_dist', True) and teacher is not None:
                with torch.no_grad():
                    t_d = teacher(drone); t_s = teacher(sat)
                l_cross = cross_dist(d_out['projected_feat'], t_d) + cross_dist(s_out['projected_feat'], t_s)
                loss += CFG.LAMBDA_CROSS_DIST * l_cross
                loss_sums['cross'] += l_cross.item()

            # SelfDistillation (conditional)
            if abl.get('use_self_dist', True):
                l_self = self_dist(d_out['cls_logits'], d_out['logits']) + self_dist(s_out['cls_logits'], s_out['logits'])
                loss += CFG.LAMBDA_SELF_DIST * l_self
                loss_sums['self'] += l_self.item()

            # UAPA (conditional)
            if abl.get('use_uapa', True):
                l_uapa = uapa(d_out['logits'], s_out['logits'])
                loss += CFG.LAMBDA_UAPA * l_uapa
