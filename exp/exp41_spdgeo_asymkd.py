...
# Novel:   Asymmetric encoder design:
#          - Gallery (satellite): DINOv2-B/14 (larger, richer) — offline indexed
#          - Query  (drone):      DINOv2-S/14 + SemanticPartDiscovery — real-time
#          - CrossSpaceBridge: projects drone embed (512) → teacher space (768)
#          - AsymmetricInfoNCE: contrastive in 768-dim teacher space
#          - SpaceAlignmentDistill: replaces CrossDistill with 768→768 alignment
#
# Deployment: gallery embeddings are pre-computed offline (ViT-B).
#             Only drone encoder (ViT-S) runs at inference → 2× faster.
# =============================================================================
...
class DINOv2GalleryEncoder(nn.Module):
    """ViT-B/14 gallery encoder — larger, richer embeddings for offline satellite indexing."""

    def __init__(self):
        super().__init__()
        print("  Loading DINOv2 ViT-B/14 gallery encoder ...")
        self.model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14", pretrained=True)
        for p in self.model.parameters():
            p.requires_grad = False
        self.gallery_proj = nn.Sequential(
            nn.Linear(768, CFG.TEACHER_DIM),
            nn.LayerNorm(CFG.TEACHER_DIM),
        )
        print("  DINOv2 ViT-B/14 gallery encoder loaded.")

    def forward(self, x):
        with torch.no_grad():
            cls_tok = self.model.forward_features(x)["x_norm_clstoken"]
        return F.normalize(self.gallery_proj(cls_tok), dim=-1)
...
class AltitudeAwarePartDiscovery(nn.Module):
    def __init__(self, feat_dim=384, n_parts=8, part_dim=256, temperature=0.07, num_altitudes=4):
        ...
        self.refine = nn.Sequential(
            nn.LayerNorm(part_dim), nn.Linear(part_dim, part_dim * 2), nn.GELU(), nn.Linear(part_dim * 2, part_dim)
        )
...
class PartAwarePooling(nn.Module):
    def __init__(self, part_dim=256, embed_dim=512):
        ...
        self.proj = nn.Sequential(
            nn.Linear(part_dim * 3, embed_dim), nn.LayerNorm(embed_dim), nn.GELU(), nn.Linear(embed_dim, embed_dim)
        )
...
class CrossSpaceBridge(nn.Module):
    """Projects drone 512-dim embed → 768-dim teacher/gallery space."""

    def __init__(self, student_dim=512, teacher_dim=768):
        super().__init__()
        self.proj = nn.Sequential(
...
class MaskedPartReconstruction(nn.Module):
    def __init__(self, part_dim=256, mask_ratio=0.30):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(part_dim, part_dim * 2), nn.GELU(), nn.Linear(part_dim * 2, part_dim), nn.LayerNorm(part_dim)
        )
...
class SPDGeoAsymKDModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        self.cross_space_bridge = CrossSpaceBridge(cfg.EMBED_DIM, cfg.TEACHER_DIM)
        ...
        print(f"  SPDGeo-AsymKD student: {total/1e6:.1f}M trainable parameters")

    def extract_embedding(self, x, alt_idx=None):
        """512-dim fused embedding (for same-space retrieval / fallback)."""
    ...
    def extract_bridged_embedding(self, x, alt_idx=None):
        """768-dim bridged embedding (for cross-space retrieval vs gallery ViT-B)."""
        fused = self.extract_embedding(x, alt_idx=alt_idx)
        return self.cross_space_bridge(fused)
    ...
    def forward(self, x, alt_idx=None, return_parts=False):
        ...
        bridged = self.cross_space_bridge(fused)

        out = {
            "embedding": fused,
            "bridged_embedding": bridged,
            "logits": logits,
...
class AsymmetricInfoNCE(nn.Module):
    """InfoNCE in teacher/gallery space: drone_bridged (768) vs sat_gallery (768)."""

    def __init__(self, temp=0.07):
        super().__init__()
        self.log_t = nn.Parameter(torch.tensor(temp).log())

    def forward(self, drone_bridged, sat_gallery_emb, labels):
        t = self.log_t.exp().clamp(0.01, 1.0)
        sim = drone_bridged @ sat_gallery_emb.T / t
        labels = labels.view(-1, 1)
        pos_mask = labels.eq(labels.T).float()
        log_prob = sim - torch.logsumexp(sim, dim=1, keepdim=True)
        loss = -(log_prob * pos_mask).sum(1) / pos_mask.sum(1).clamp(min=1)
...
class SpaceAlignmentLoss(nn.Module):
    """Align drone bridged embedding with gallery encoder embedding."""

    def forward(self, drone_bridged, gallery_emb):
        s = F.normalize(drone_bridged, dim=-1)
        t = F.normalize(gallery_emb, dim=-1)
        mse = F.mse_loss(s, t)
...
# =============================================================================
# EVALUATION — asymmetric: drone(bridged 768) vs gallery(ViT-B 768)
# =============================================================================


@torch.no_grad()
def evaluate(model, gallery_encoder, test_ds, device):
    model.eval()
    gallery_encoder.eval()
    ...
    for b in loader:
        ...
        feat = model.extract_bridged_embedding(b["drone"].to(device), alt_idx=alt_idx).cpu()
    ...
    for i in range(0, len(sat_img_list), 64):
        ...
        sat_feats.append(gallery_encoder(batch).cpu())
    sat_feats = torch.cat(sat_feats)
    sat_labels = torch.tensor(sat_label_list)

    print(f"  Gallery: {len(sat_feats)} sat (ViT-B, 768d) | Queries: {len(drone_feats)} drone (bridged, 768d)")
    ...
    return {"R@1": r1, "R@5": r5, "R@10": r10, "mAP": ap}, per_alt
...
def train_one_epoch(model, gallery_encoder, ema, loader, losses, new_losses, optimizer, scaler, device, epoch):
    model.train()
    gallery_encoder.eval()

    infonce, ce, consist, self_dist, uapa = losses
    proxy_anchor, ema_dist, alt_consist, diversity_loss, asym_nce, space_align = new_losses
    ...
    for batch in tqdm(loader, desc=f"Ep{epoch:3d}", leave=False):
        ...
        with torch.amp.autocast("cuda", enabled=CFG.USE_AMP):
            ...
            # Gallery encoder: satellite through ViT-B
            with torch.no_grad():
                sat_gallery_emb = gallery_encoder(sat)
            ...
            # Asymmetric: drone bridged vs gallery ViT-B
            l_asym = asym_nce(d_out["bridged_embedding"], sat_gallery_emb, labels)
            l_align = space_align(d_out["bridged_embedding"], sat_gallery_emb)
            ...
            if recon_active:
                l_recon_d = model.mask_recon(d_out["parts"]["projected_patches"], d_out["parts"]["part_features"], d_out["parts"]["assignment"])
                l_recon_s = model.mask_recon(s_out["parts"]["projected_patches"], s_out["parts"]["part_features"], s_out["parts"]["assignment"])
            ...
            loss = (
                ...
                + CFG.LAMBDA_ASYM_NCE * l_asym
                + CFG.LAMBDA_SPACE_ALIGN * l_align
            )
        ...
        loss_sums["asym"] += l_asym.item()
        loss_sums["align"] += l_align.item()
...
def main():
    ...
    print("=" * 65)
    print("  EXP41: SPDGeo-AsymKD — Asymmetric Encoder (ViT-B gallery / ViT-S drone)")
    ...
    print("\nBuilding models ...")
    model = SPDGeoAsymKDModel(CFG.NUM_CLASSES).to(DEVICE)
    gallery_encoder = DINOv2GalleryEncoder().to(DEVICE)
    gallery_encoder.eval()
    ...
    base_losses = (infonce, ce, consist, self_dist, uapa_loss)
    ...
    asym_nce = AsymmetricInfoNCE(temp=0.07).to(DEVICE)
    space_align = SpaceAlignmentLoss()
    new_losses = (proxy_anchor, ema_dist, alt_consist, diversity_loss, asym_nce, space_align)

    backbone_params = [p for p in model.backbone.parameters() if p.requires_grad]
    head_params = [p for n, p in model.named_parameters() if p.requires_grad and not n.startswith("backbone")]
    gallery_trainable = [p for p in gallery_encoder.parameters() if p.requires_grad]

    optimizer = torch.optim.AdamW(
        [
            {"params": backbone_params, "lr": CFG.BACKBONE_LR},
            {"params": head_params, "lr": CFG.LR},
            {"params": gallery_trainable, "lr": CFG.LR * 0.1},
            {"params": infonce.parameters(), "lr": CFG.LR},
            {"params": proxy_anchor.parameters(), "lr": CFG.LR},
            {"params": asym_nce.parameters(), "lr": CFG.LR},
        ],
    ...
    ckpt_path = os.path.join(CFG.OUTPUT_DIR, "exp41_spdgeo_asymkd_best.pth")
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        avg_loss, ld = train_one_epoch(
            model, gallery_encoder, ema, train_loader, base_losses, new_losses, optimizer, scaler, DEVICE, epoch
        )
        ...
        print(
            ...
            f"Asym {ld['asym']:.3f} Align {ld['align']:.3f} {recon_tag}"
        )

        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            metrics, per_alt = evaluate(model, gallery_encoder, test_ds, DEVICE)
            ...
            if metrics["R@1"] > best_r1:
                ...
                torch.save(
                    {"epoch": epoch, "model_state_dict": model.state_dict(),
                     "gallery_state_dict": gallery_encoder.state_dict(),
                     "metrics": metrics, "per_alt": per_alt},
                    ckpt_path,
                )
                print(f"  [BEST] Epoch {epoch} — R@1 {best_r1:.2f}% (checkpoint saved)")

    with open(os.path.join(CFG.OUTPUT_DIR, "exp41_spdgeo_asymkd_log.json"), "w") as f:
