"""
EXP9: GeoCLIP — CLIP ViT-L/14 Teacher + Text-Guided Cross-View Alignment
=========================================================================
Teacher:  CLIP ViT-L/14 (428M params, 768-dim) via open_clip
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) CLIP visual encoder as teacher — semantically aware features
          2) Text-Guided Cross-View Loss — use text prompts to anchor views
          3) Semantic Consistency Distillation — match CLIP's semantic structure
Expected: 78-83% R@1
"""
...
for p in ["timm","tqdm","open_clip_torch"]:
    try: importlib.import_module(p.replace("-","_").replace("_torch",""))
...
class Config:
    ...
    LAMBDA_TEXT_GUIDE=0.15   # Text-guided cross-view loss
    LAMBDA_SEM_CONSIST=0.15  # Semantic consistency distillation
...
# ============================================================================
# CLIP VIT-L/14 TEACHER + TEXT ENCODER
# ============================================================================
class CLIPTeacher(nn.Module):
    """CLIP ViT-L/14 Teacher with text encoder for text-guided distillation"""
    def __init__(self, ntb=2):
        super().__init__()
        import open_clip
        print("Loading CLIP ViT-L/14 teacher via open_clip...")
        self.clip_model, _, self.clip_preprocess = open_clip.create_model_and_transforms(
            'ViT-L-14', pretrained='openai')
        self.tokenizer = open_clip.get_tokenizer('ViT-L-14')
        self.num_channels = 768  # CLIP ViT-L/14 outputs 768-dim

        # Freeze visual encoder
        for p in self.clip_model.parameters(): p.requires_grad = False
        # Unfreeze last N blocks of visual encoder
        visual = self.clip_model.visual
        if hasattr(visual, 'transformer') and hasattr(visual.transformer, 'resblocks'):
            for blk in visual.transformer.resblocks[-ntb:]:
                for p in blk.parameters(): p.requires_grad = True
        elif hasattr(visual, 'blocks'):
            for blk in visual.blocks[-ntb:]:
                for p in blk.parameters(): p.requires_grad = True

        # Pre-compute text embeddings for drone and satellite descriptions
        self._precompute_text_embeddings()

        t=sum(p.numel() for p in self.clip_model.parameters())
        tr=sum(p.numel() for p in self.clip_model.parameters() if p.requires_grad)
        print(f"  CLIP ViT-L/14: {t/1e6:.0f}M total, {tr/1e6:.1f}M trainable")

    def _precompute_text_embeddings(self):
        """Pre-compute text embeddings for view-specific prompts"""
        drone_texts = [
            "an aerial drone photograph of a location",
            "a bird's eye view drone image of an area",
            "a drone captured overhead view of buildings and streets",
        ]
        sat_texts = [
            "a satellite image of a geographic location",
            "a top-down satellite view of an area",
            "a satellite captured overhead map of buildings and streets",
        ]
        with torch.no_grad():
            drone_tokens = self.tokenizer(drone_texts)
            sat_tokens = self.tokenizer(sat_texts)
            self.register_buffer('drone_text_feat',
                F.normalize(self.clip_model.encode_text(drone_tokens).mean(0, keepdim=True), dim=1))
            self.register_buffer('sat_text_feat',
                F.normalize(self.clip_model.encode_text(sat_tokens).mean(0, keepdim=True), dim=1))
        print("  Pre-computed text embeddings for drone/satellite prompts")

    @torch.no_grad()
    def forward(self, x):
        return self.clip_model.encode_image(x).float()  # [B, 768]

    def get_text_features(self):
        return self.drone_text_feat, self.sat_text_feat
...
class CrossDistillationLoss(nn.Module):
    def __init__(self, sd=768, td=768):
        super().__init__(); self.proj=nn.Linear(sd,td) if sd!=td else nn.Identity()
    def forward(self,sf,tf):
        s=F.normalize(self.proj(sf),1); t=F.normalize(tf,1)
        return F.mse_loss(s,t)+(1-F.cosine_similarity(s,t).mean())

# === NEW: Text-Guided Cross-View Loss ===
class TextGuidedCVLoss(nn.Module):
    """Use CLIP text embeddings to guide cross-view alignment.
    Drone features should be close to 'drone text' and satellite features
    close to 'satellite text', creating a semantic bridge."""
    def __init__(self, student_dim=768, clip_dim=768):
        super().__init__()
        self.proj = nn.Linear(student_dim, clip_dim) if student_dim != clip_dim else nn.Identity()

    def forward(self, student_feat, text_feat):
        s = F.normalize(self.proj(student_feat), dim=1)  # [B, D]
        t = text_feat.expand(s.size(0), -1)  # [B, D]
        return 1 - F.cosine_similarity(s, t).mean()

# === NEW: Semantic Consistency Distillation ===
class SemanticConsistencyLoss(nn.Module):
    """Force student to maintain CLIP's semantic similarity structure.
    If CLIP says two images are semantically similar, student should agree."""
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.cd=CrossDistillationLoss(768, 768)  # CLIP ViT-L outputs 768
        self.text_guide=TextGuidedCVLoss(768, 768)
        self.sem_consist=SemanticConsistencyLoss()

    def forward(self,do,so,l,td=None,ts=None,drone_text=None,sat_text=None):
        losses={}
        ...
        if td is not None:
            ...
            if drone_text is not None:
                losses['text_guide']=self.cfg.LAMBDA_TEXT_GUIDE*(
                    self.text_guide(do['final_feature'],drone_text)+self.text_guide(so['final_feature'],sat_text))
            # Semantic consistency
            losses['sem_consist']=self.cfg.LAMBDA_SEM_CONSIST*(
                self.sem_consist(do['embedding_normed'],td)+self.sem_consist(so['embedding_normed'],ts))
...
def train_one_epoch(model,teacher,tl,crit,opt,scaler,dev,epoch,cfg=Config):
    ...
    drone_text, sat_text = (teacher.get_text_features() if teacher else (None, None))
    for bi,b in enumerate(tl):
        ...
        with torch.amp.autocast('cuda',enabled=cfg.USE_AMP):
            ...
            loss,ld=crit(do,so,l,td,ts,drone_text,sat_text)
...
def main():
    ...
    print("="*60+"\nEXP9: GeoCLIP — CLIP ViT-L/14 + Text-Guided Alignment\n"+"="*60)
    ...
    try: teacher=CLIPTeacher(2).to(dev)
    except Exception as e: print(f"CLIP failed:{e}"); teacher=None
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp9.pth')); print("  ★ Best!")
