# =============================================================================
# EXP26: SPDGeo-DPEA — DPE + Deep Altitude-Adaptive Parts
# =============================================================================
# Base:    SPDGeo-DPE (93.59% R@1) — THE CHAMPION
# Novel:   Combines DPE's 3 proven components with Deep Altitude FiLM:
#          1) ProxyAnchor Loss (from DPE — replaces saturating Triplet)
#          2) DynamicFusionGate (from DPE — adaptive part/cls weighting)
#          3) EMA Teacher Ensemble (from DPE — smooth distillation)
#          4) DeepAltitudeFiLM — FiLM conditioning INSIDE part discovery
#             (fixes EXP19's root cause: post-aggregation FiLM too shallow)
#          5) AltitudeConsistencyLoss — prevent altitude views from diverging
#
# Motivation:
#   EXP20 (DPE) achieved 93.59% R@1 by fixing loss saturation + fusion.
#   EXP19 (AAP) achieved 91.75% R@1 by adding altitude conditioning.
#   These address DIFFERENT weaknesses → potentially additive:
#   - DPE fixes: triplet saturation, fixed fusion, teacher oscillation
#   - AAP fixes: altitude-blind part discovery (150m gap still 6.92% in DPE)
#
#   KEY UPGRADE over EXP19: FiLM is applied INSIDE SemanticPartDiscovery's
#   feat_proj output, BEFORE prototype similarity computation. This means
#   altitude reshapes WHICH patches get assigned to WHICH parts, not just
#   post-hoc reweighting of already-aggregated features.
#
#   Real-world impact: Every drone system has altitude metadata from GPS/
#   barometer — this is zero-cost information that should be exploited.
#
# Architecture: SPDGeo-DPE + DeepAltitudeFiLM + AltitudeConsistencyLoss
# Total losses: 9 (6 DPE base + ProxyAnchor + EMA + AltConsist)
# =============================================================================
...
class Config:
    ...
    # Base loss weights (from DPE — no triplet)
    LAMBDA_CE           = 1.0
    LAMBDA_INFONCE      = 1.0
...
# =============================================================================
# DATASET — returns altitude index for FiLM conditioning
# =============================================================================
...
# =============================================================================
# NEW: Deep Altitude FiLM — conditions INSIDE part discovery
# =============================================================================
class DeepAltitudeFiLM(nn.Module):
    """
    Feature-wise Linear Modulation conditioned on altitude.

    KEY DIFFERENCE from EXP19 (AAP):
      EXP19 applied FiLM AFTER part aggregation (post_feat = γ * part_feat + β).
      This only reweights already-aggregated features — altitude can't change
      which patches get assigned to which parts.

      Here, FiLM is applied to the feat_proj output BEFORE prototype similarity,
      so altitude influences the assignment step itself:
        feat_proj → FiLM(feat, alt) → prototype_sim → softmax → part_feat

    For satellite images (no altitude), we use mean(γ, β) across all altitudes.
    This is elegant: satellite represents the "average viewpoint" naturally.

    Params: NUM_ALT × part_dim × 2 (γ + β) = 4 × 256 × 2 = 2,048 params
    """
    def __init__(self, num_altitudes=4, feat_dim=256):
        super().__init__()
        self.num_altitudes = num_altitudes
        # Per-altitude affine parameters
    ...
    def forward(self, feat, alt_idx=None):
        """
        feat: [B, N, D] — projected patch features
        alt_idx: [B] int tensor — altitude index (0-3) for drone, None for satellite
        Returns: [B, N, D] modulated features
        """
        if alt_idx is None:
            # Satellite: use mean across all altitudes
            gamma = self.gamma.mean(dim=0, keepdim=True)  # [1, D]
            beta  = self.beta.mean(dim=0, keepdim=True)    # [1, D]
            return feat * gamma.unsqueeze(0) + beta.unsqueeze(0)
        else:
            # Drone: use altitude-specific parameters
            gamma = self.gamma[alt_idx]  # [B, D]
            beta  = self.beta[alt_idx]   # [B, D]
...
class AltitudeAwarePartDiscovery(nn.Module):
    """
    SemanticPartDiscovery with DeepAltitudeFiLM injected at the right place.

    Pipeline:
      patch_features → feat_proj → **DeepFiLM(alt)** → proto_sim → assign →
      → weighted_aggregate → refine → part_features

    The FiLM modulation happens BEFORE prototype similarity, so altitude
    reshapes which patches are clustered together.
    """
    def __init__(self, feat_dim=384, n_parts=8, part_dim=256, temperature=0.07,
        ...
        # Deep FiLM: modulates feat_proj output before prototype similarity
    ...
    def forward(self, patch_features, spatial_hw, alt_idx=None):
        """
        patch_features: [B, N, 384]
        spatial_hw: (H, W)
        alt_idx: [B] int tensor or None (satellite)
        """
        ...
        # === DEEP ALTITUDE FiLM — applied BEFORE prototype similarity ===
        feat = self.altitude_film(feat, alt_idx)

        # Prototype similarity and soft assignment
        ...
        part_feat = torch.bmm(assign_t, feat) / mass  # [B, K, part_dim]
...
# =============================================================================
# STUDENT MODEL — SPDGeo-DPEA
# =============================================================================
class SPDGeoDPEAModel(nn.Module):
    """
    SPDGeo-DPE + Deep Altitude-Adaptive Parts:
    1. DynamicFusionGate (from DPE)
    2. AltitudeAwarePartDiscovery with DeepFiLM (NEW — fixes EXP19's limitation)
    3. ProxyAnchor + EMA (in loss/training, not model)
    """
    def __init__(self, num_classes, cfg=CFG):
        super().__init__()
        self.backbone  = DINOv2Backbone(cfg.UNFREEZE_BLOCKS)
        # NEW: Altitude-aware part discovery with Deep FiLM
        ...
        print(f"  SPDGeo-DPEA student: {total/1e6:.1f}M trainable parameters")
...
class AltitudeConsistencyLoss(nn.Module):
    """
    Ensures embeddings of the same location at different altitudes remain close.
    Within each batch, finds samples sharing the same location label but
    different altitudes, and minimizes pairwise cosine distance.

    This prevents DeepFiLM from pushing altitude-specific views apart while
    trying to specialize part assignments per altitude.
    """
    def forward(self, embeddings, labels, altitudes):
        ...
        # Find pairs: same location, different altitude
        lbl = labels.view(-1, 1)
        alt = altitudes.view(-1, 1)
        same_loc = lbl.eq(lbl.T)        # [B, B]
        diff_alt = alt.ne(alt.T)          # [B, B]
        mask = same_loc & diff_alt        # same location, different altitude

        if mask.sum() == 0:
            return torch.tensor(0.0, device=embeddings.device)

        # Cosine distance for valid pairs
        emb_norm = F.normalize(embeddings, dim=-1)
        cos_sim = emb_norm @ emb_norm.T   # [B, B]
        cos_dist = 1 - cos_sim             # [B, B]

        # Mean cosine distance for altitude-variant pairs of same location
...
def evaluate(model, test_ds, device):
    ...
    # Satellite: no altitude → alt_idx=None → uses mean FiLM params
...
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            # Drone: pass altitude index for DeepFiLM conditioning
            d_out = model(drone, alt_idx=alt_idx, return_parts=True)
            # Satellite: no altitude → uses mean FiLM params
...
def main():
    ...
    print("=" * 65)
    print("  EXP26: SPDGeo-DPEA — DPE + Deep Altitude-Adaptive Parts")
    print(f"  Base: SPDGeo-DPE (93.59% R@1)")
    print(f"  Novel: DeepAltitudeFiLM (inside part discovery) + AltConsist")
    print(f"  Inherited: ProxyAnchor + DynamicFusionGate + EMA")
    ...
    print(f"  Losses: 6 base + Proxy + EMA + AltConsist = 9 total")
    print("=" * 65)
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"UAPA {ld['uapa']:.3f} | "
              f"Proxy {ld['proxy']:.3f}  EMA {ld['ema']:.3f}  Alt {ld['alt']:.3f} | "
              f"LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp26_dpea_best.pth'))
    ...
    print(f'\n{"="*65}')
    print(f'  EXP26: SPDGeo-DPEA COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp26_dpea_results.json'), 'w') as f:
