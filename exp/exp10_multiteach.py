"""
EXP10: GeoMultiTeach — Dual-Teacher Ensemble Distillation
==========================================================
Teachers: DINOv2 ViT-B/14 (86M, 768d) + BEiTv2-Large (304M, 1024d)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) Dual-teacher distillation — learn from 2 complementary teachers
          2) Confidence-Weighted Fusion — trust the more confident teacher per sample
          3) Teacher Agreement Bonus — extra weight when both teachers agree
Expected: 81-85% R@1
"""
...
class Config:
    ...
    LAMBDA_AGREE=0.15  # Teacher agreement bonus
...
class DINOv2Teacher(nn.Module):
    """DINOv2 ViT-B/14 Teacher (same as baseline)"""
    def __init__(self, ntb=2):
        ...
        print(f"  DINOv2-Base: 768-dim output")
    @torch.no_grad()
    def forward(self,x):
...
class BEiTv2Teacher(nn.Module):
    """BEiTv2-Large Teacher"""
    def __init__(self, ntb=2):
        super().__init__()
        print("Loading BEiTv2-Large teacher...")
        ...
        if hasattr(self.model,'blocks'):
            for blk in self.model.blocks[-ntb:]:
                for p in blk.parameters(): p.requires_grad=True
        print(f"  BEiTv2-Large: 1024-dim output")
    @torch.no_grad()
...
class DualTeacherDistillLoss(nn.Module):
    """Confidence-Weighted Dual-Teacher Distillation.
    Student learns from both teachers, weighted by their confidence per sample.
    When teachers agree, an extra bonus is applied."""
    def __init__(self, student_dim=768, t1_dim=768, t2_dim=1024):
        super().__init__()
        self.proj1 = nn.Linear(student_dim, t1_dim) if student_dim != t1_dim else nn.Identity()
        self.proj2 = nn.Linear(student_dim, t2_dim) if student_dim != t2_dim else nn.Identity()

    def _confidence(self, feat):
        """Feature norm as proxy for confidence"""
        return feat.norm(dim=1, keepdim=True)

    def forward(self, student_feat, t1_feat, t2_feat):
        # Project student to each teacher's dim
        s1 = F.normalize(self.proj1(student_feat), dim=1)
        s2 = F.normalize(self.proj2(student_feat), dim=1)
        t1 = F.normalize(t1_feat, dim=1)
        t2 = F.normalize(t2_feat, dim=1)

        # Per-sample confidence weights
        with torch.no_grad():
            c1 = self._confidence(t1_feat)  # [B, 1]
            c2 = self._confidence(t2_feat)  # [B, 1]
            w1 = c1 / (c1 + c2 + 1e-8)
            w2 = c2 / (c1 + c2 + 1e-8)

        # Weighted distillation
        loss1 = (w1 * (1 - F.cosine_similarity(s1, t1, dim=1, eps=1e-8).unsqueeze(1))).mean()
        loss2 = (w2 * (1 - F.cosine_similarity(s2, t2, dim=1, eps=1e-8).unsqueeze(1))).mean()

        return loss1 + loss2

class TeacherAgreementLoss(nn.Module):
    """Extra bonus when both teachers agree on relative similarity.
    If t1 says samples i,j are similar AND t2 agrees, trust more."""
    ...
    def forward(self, student_feat, t1_feat, t2_feat):
        ...
        with torch.no_grad():
            ...
            t1_sim = t1 @ t1.T
            t2_sim = t2 @ t2.T
            # Agreement mask: both teachers assign high similarity
            agreement = (t1_sim * t2_sim).clamp(min=0)  # Element-wise, positive = agreement
            agreement = agreement / (agreement.sum(1, keepdim=True) + 1e-8)

        s_sim = s @ s.T / self.t
        s_log_prob = F.log_softmax(s_sim, dim=1)
        return F.kl_div(s_log_prob, agreement, reduction='batchmean')
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.dual_dist = DualTeacherDistillLoss(768, 768, 1024)
        self.agree = TeacherAgreementLoss()

    def forward(self,do,so,l,t1d=None,t1s=None,t2d=None,t2s=None):
        losses={}
        ...
        if t1d is not None and t2d is not None:
            losses['dual_dist']=self.cfg.LAMBDA_CROSS_DIST*(
                self.dual_dist(do['final_feature'],t1d,t2d)+self.dual_dist(so['final_feature'],t1s,t2s))
            losses['agree']=self.cfg.LAMBDA_AGREE*(
                self.agree(do['embedding_normed'],t1d,t2d)+self.agree(so['embedding_normed'],t1s,t2s))
...
def train_one_epoch(model,t1,t2,tl,crit,opt,scaler,dev,epoch,cfg=Config):
    model.train()
    if t1: t1.eval()
    if t2: t2.eval()
    ...
    for bi,b in enumerate(tl):
        ...
        with torch.amp.autocast('cuda',enabled=cfg.USE_AMP):
            ...
            t1d=t1s=t2d=t2s=None
            if t1:
                with torch.no_grad(): t1d=t1(di); t1s=t1(si)
            if t2:
                with torch.no_grad(): t2d=t2(di); t2s=t2(si)
            loss,ld=crit(do,so,l,t1d,t1s,t2d,t2s)
...
def main():
    ...
    print("="*60+"\nEXP10: GeoMultiTeach — Dual-Teacher Ensemble\n"+"="*60)
    ...
    t1=t2=None
    try: t1=DINOv2Teacher(2).to(dev)
    except Exception as e: print(f"DINOv2 failed:{e}")
    try: t2=BEiTv2Teacher(2).to(dev)
    except Exception as e: print(f"BEiTv2 failed:{e}")
    ...
    for ep in range(Config.NUM_EPOCHS):
        ...
        al,ld=train_one_epoch(model,t1,t2,trl,crit,opt,scaler,dev,ep)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp10.pth')); print("  ★ Best!")
