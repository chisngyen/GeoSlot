"""
EXP13: GeoPolar — Polar Transform + Rotation-Invariant Augmentation
=====================================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) Polar Transform on satellite images — bridges geometric gap to drone view
          2) Heavy rotation augmentation on drone images (0-360° random rotation)
          3) Rotation-Invariant Contrastive Loss — invariant to angular transformation
Expected: 82-85% R@1 (addresses fundamental geometric mismatch)
"""
...
class Config:
    ...
    LAMBDA_ROT_INV=0.3  # Rotation-invariant consistency
    MARGIN=0.3; ALTITUDES=["150","200","250","300"]
    TRAIN_LOCS=list(range(1,121)); TEST_LOCS=list(range(121,201))
    USE_AMP=True; SEED=42; USE_POLAR=True  # Enable polar transform
...
# === POLAR TRANSFORM ===
def polar_transform(img_tensor, center=None):
    """Apply polar coordinate transform to image tensor [C,H,W] → [C,H,W].
    Converts Cartesian satellite view to polar coordinates,
    bridging geometric gap to drone's radial perspective."""
    ...
    radius = torch.linspace(0, max_radius, H, device=img_tensor.device)
    r_grid, t_grid = torch.meshgrid(radius, theta, indexing='ij')

    # Convert to Cartesian
    x_grid = (cx + r_grid * torch.cos(t_grid))
    y_grid = (cy + r_grid * torch.sin(t_grid))
    ...
    polar_img = F.grid_sample(img, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
    return polar_img.squeeze(0)  # [C, H, W]


class PolarTransform:
    """Apply polar transform as a torchvision-style transform"""
    def __call__(self, img_tensor):
        return polar_transform(img_tensor)


# === DATASET (with polar transform for satellite) ===
class SUES200Dataset(Dataset):
    def __init__(self, root, mode="train", altitudes=None, transform=None,
                 train_locs=None, test_locs=None, use_polar=False):
        self.root=root; self.mode=mode; self.altitudes=altitudes or Config.ALTITUDES
        self.transform=transform; self.use_polar=use_polar
        ...
        print(f"[{mode}] {len(self.samples)} samples, {len(self.locations)} locs (polar={use_polar})")
    def __len__(self): return len(self.samples)
    def __getitem__(self, idx):
        dp,sp,li,alt=self.samples[idx]
        d=Image.open(dp).convert('RGB'); s=Image.open(sp).convert('RGB')
        if self.transform:
            d=self.transform(d); s=self.transform(s)
        # Apply polar transform to satellite image
        if self.use_polar:
            s = polar_transform(s)
...
def get_transforms(mode="train"):
    """Enhanced transforms with heavier rotation for drone images"""
    if mode=="train":
        return T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),
            T.RandomHorizontalFlip(0.5),
            T.RandomRotation(180),  # Heavy rotation — drone views can be any angle
...
# === BACKBONE + STUDENT + TEACHER (same as baseline, compact) ===
...
class MobileGeoStudent(nn.Module):
    ...
    def forward(self, x, return_all=False):
        ...
        if return_all:
            return {'embedding':emb,'embedding_normed':en,'logits':logits,'stage_logits':sl,'stage_features':so,'final_feature':ff}
        return en, logits
...
# === NEW: Rotation-Invariant Consistency Loss ===
class RotationInvariantLoss(nn.Module):
    """Ensures model produces same features regardless of rotation.
    Processes same image at 2 random rotations, minimizes feature distance."""
    def forward(self, feat1, feat2):
        """feat1, feat2: features from same images with different rotations"""
        f1 = F.normalize(feat1, dim=1)
        f2 = F.normalize(feat2, dim=1)
        return (1 - F.cosine_similarity(f1, f2)).mean()
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.rot_inv=RotationInvariantLoss()
    def forward(self,do,so,l,td=None,ts=None,do_rot=None):
        losses={}
        ...
        # Rotation-invariant consistency (drone features should be same after rotation)
        if do_rot is not None:
            losses['rot_inv']=self.cfg.LAMBDA_ROT_INV*self.rot_inv(do['embedding_normed'],do_rot['embedding_normed'])
...
def train_one_epoch(model,teacher,tl,crit,opt,scaler,dev,epoch,cfg=Config):
    ...
    rot_transform = T.RandomRotation(180)  # for rotation consistency
    for bi,b in enumerate(tl):
        ...
        with torch.amp.autocast('cuda',enabled=cfg.USE_AMP):
            ...
            # Rotation-invariant: rotate drone and get features
            with torch.no_grad():
                di_rot = torch.stack([rot_transform(img) for img in di])  # rotate each image
            do_rot=model(di_rot, return_all=True)
            td=ts=None
            if teacher:
                with torch.no_grad(): td=teacher(di); ts=teacher(si)
            loss,ld=crit(do,so,l,td,ts,do_rot)
...
def main():
    ...
    print("="*60+"\nEXP13: GeoPolar — Polar Transform + Rotation Invariance\n"+"="*60)
    set_seed(Config.SEED); dev=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    trd=SUES200Dataset(Config.DATA_ROOT,"train",transform=get_transforms("train"),use_polar=Config.USE_POLAR)
    ted=SUES200Dataset(Config.DATA_ROOT,"test",transform=get_transforms("test"),use_polar=False)  # No polar at test (single view)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp13.pth')); print("  ★ Best!")
