"""
EXP15: GeoCKD — Comparative Knowledge Distillation
=====================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) Comparative KD — learn from DIFFERENCES between sample pairs
          2) Cross-View Comparative Loss — compare drone-satellite differences
          3) Hard Negative Comparative Mining
Ref:      CKD (WACV 2025) — adapted for cross-view geo-localization
Expected: 82-86% R@1 (lightweight, theoretically motivated)
"""
...
class Config:
    ...
    LAMBDA_CKD=0.3        # Comparative KD
    LAMBDA_CV_COMP=0.2     # Cross-View Comparative
...
# === BACKBONE + STUDENT + TEACHER (compact inline) ===
...
# ============================================================================
# NEW: Comparative Knowledge Distillation (WACV 2025 inspired)
# ============================================================================
class ComparativeKDLoss(nn.Module):
    """Transfer pairwise DIFFERENCE representations from teacher to student.
    Instead of matching f_s ≈ f_t, match (f_s_i - f_s_j) ≈ (f_t_i - f_t_j).
    This captures how the teacher distinguishes between samples.
    
    Key insight: Two locations may have similar absolute features,
    but what DIFFERENTIATES them is crucial for retrieval.
    """
    def __init__(self, dim=768, proj_dim=256):
        super().__init__()
        self.diff_proj = nn.Sequential(
            nn.Linear(dim, proj_dim), nn.ReLU(), nn.Linear(proj_dim, proj_dim))

    def forward(self, student_feats, teacher_feats, labels):
        B = student_feats.size(0)
        if B < 4: return torch.tensor(0., device=student_feats.device)

        # Sample pairs: positive (same class) and negative (different class)
        labels = labels.view(-1)
        # Compute all pairwise differences
        s_diff = student_feats.unsqueeze(0) - student_feats.unsqueeze(1)  # [B, B, D]
        with torch.no_grad():
            t_diff = teacher_feats.unsqueeze(0) - teacher_feats.unsqueeze(1)  # [B, B, D]

        # Project differences (only upper triangle to avoid redundancy)
        idx = torch.triu_indices(B, B, offset=1)
        s_pairs = self.diff_proj(s_diff[idx[0], idx[1]])  # [N_pairs, proj_dim]
        with torch.no_grad():
            t_pairs = F.normalize(t_diff[idx[0], idx[1]], dim=1)

        s_pairs_norm = F.normalize(s_pairs, dim=1)
        return F.mse_loss(s_pairs_norm, t_pairs)


class CrossViewComparativeLoss(nn.Module):
    """Compare drone-satellite difference structures.
    For same location: drone_feat - sat_feat should be similar in teacher and student.
    This captures the VIEW-SPECIFIC transformation between views."""
    def __init__(self):
        super().__init__()

    def forward(self, drone_s, sat_s, drone_t, sat_t, labels):
        """Cross-view difference alignment"""
        # Same-location pairs
        ds = F.normalize(drone_s - sat_s, dim=1)  # student view-difference
        with torch.no_grad():
            dt = F.normalize(drone_t - sat_t, dim=1)  # teacher view-difference
        return (1 - F.cosine_similarity(ds, dt)).mean()
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.ckd=ComparativeKDLoss()
        self.cv_comp=CrossViewComparativeLoss()
    def forward(self,do,so,l,td=None,ts=None):
        ...
        if td is not None:
            ...
            losses['ckd']=self.cfg.LAMBDA_CKD*(
                self.ckd(do['final_feature'],td,l)+self.ckd(so['final_feature'],ts,l))
            losses['cv_comp']=self.cfg.LAMBDA_CV_COMP*self.cv_comp(
                do['final_feature'],so['final_feature'],td,ts,l)
...
def main():
    ...
    print("="*60+"\nEXP15: GeoCKD — Comparative Knowledge Distillation (WACV'25)\n"+"="*60)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp15.pth')); print("  ★ Best!")
