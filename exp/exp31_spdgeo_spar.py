...
# Novel:   1) PartRelationTransformer — 2-layer self-attention over K=8 parts
#             with SPATIAL positional encoding from part_positions. Captures
#             inter-part co-occurrence ("road-near-building" structure).
#          2) RelationContrastiveLoss — aligns K×K relation graphs across
#             drone/satellite views via softmax-normalized similarity matrices.
#
# Motivation:
#   In DPE, parts are pooled as a bag (attn+mean+max) — their spatial layout
#   is IGNORED. But geo-localization relies on spatial configuration:
#   "park left of river with building above" is location-specific.
#   A lightweight transformer over parts captures these relational patterns.
#
#   This is DIFFERENT from EXP35 (CRA) which aligns relation matrices.
#   SPAR enriches part features WITH relational context BEFORE pooling.
#   CRA adds a matching loss on frozen relations. SPAR changes the
#   representation; CRA changes the objective.
#
# Architecture: SPDGeo-DPE + PartRelationTransformer + RelationContrastiveLoss
# Total losses: 9 (8 DPE base + 1 new RelationContrastive)
# =============================================================================
...
class Config:
    ...
    # NEW: Part Relation Transformer
    RELATION_HEADS      = 4     # multi-head attention heads
    RELATION_LAYERS     = 2     # transformer layers
    RELATION_DROPOUT    = 0.1
    LAMBDA_RELATION     = 0.25  # relation contrastive loss weight
...
# =============================================================================
# SEMANTIC PART DISCOVERY (same as DPE)
# =============================================================================
...
class SpatialPositionalEncoding(nn.Module):
    """
    Encode part spatial positions (x, y) ∈ [0, 1]² into d-dimensional vectors
    using sinusoidal encoding, then project to part_dim.
    """
    def __init__(self, part_dim=256, pos_dim=64):
        super().__init__()
        self.pos_dim = pos_dim
        # Sinusoidal frequency bands
        freq = torch.arange(0, pos_dim // 2, dtype=torch.float32)
        freq = 1.0 / (10000.0 ** (freq / (pos_dim // 2)))
        self.register_buffer('freq', freq)
        self.proj = nn.Linear(pos_dim * 2, part_dim)

    def forward(self, positions):
        """positions: [B, K, 2] normalized (x, y) in [0,1]"""
        B, K, _ = positions.shape
        x = positions[..., 0:1]  # [B, K, 1]
        y = positions[..., 1:2]  # [B, K, 1]
        freq = self.freq.view(1, 1, -1)  # [1, 1, pos_dim//2]
        enc_x = torch.cat([torch.sin(x * freq * math.pi), torch.cos(x * freq * math.pi)], dim=-1)
        enc_y = torch.cat([torch.sin(y * freq * math.pi), torch.cos(y * freq * math.pi)], dim=-1)
        enc = torch.cat([enc_x, enc_y], dim=-1)  # [B, K, pos_dim*2]
        return self.proj(enc)  # [B, K, part_dim]


class PartRelationTransformer(nn.Module):
    """
    Lightweight transformer over K=8 discovered parts.

    Input:  part_features [B, K, D] + part_positions [B, K, 2]
    Output: relation-enriched part features [B, K, D]

    The transformer adds spatial positional encoding from part positions,
    then runs L layers of self-attention. This allows parts to attend to
    each other based on BOTH semantic similarity and spatial proximity.

    After the transformer, each part feature encodes not just "what" it is
    but also its spatial relationship to other parts — capturing layout
    patterns like "road adjacent to building" that are location-discriminative.
    """
    def __init__(self, part_dim=256, n_heads=4, n_layers=2, dropout=0.1):
        super().__init__()
        self.pos_enc = SpatialPositionalEncoding(part_dim)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=part_dim, nhead=n_heads, dim_feedforward=part_dim * 2,
            dropout=dropout, activation='gelu', batch_first=True, norm_first=True
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)
        self.norm = nn.LayerNorm(part_dim)

    def forward(self, part_features, part_positions):
        """
        part_features:  [B, K, D]
        part_positions: [B, K, 2]
        Returns:        [B, K, D] relation-enriched features
        """
        pos_emb = self.pos_enc(part_positions)   # [B, K, D]
        x = part_features + pos_emb               # Add spatial info
        x = self.transformer(x)                    # Self-attention over parts
        return self.norm(x + part_features)        # Residual connection


# =============================================================================
# PART-AWARE POOLING + FUSION (same as DPE)
# =============================================================================
...
# STUDENT MODEL
# =============================================================================
class SPDGeoSPARModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        # NEW: Part Relation Transformer — enriches parts with spatial context
        self.part_rel    = PartRelationTransformer(cfg.PART_DIM, cfg.RELATION_HEADS,
                                                    cfg.RELATION_LAYERS, cfg.RELATION_DROPOUT)
        ...
        print(f"  SPDGeo-SPAR student: {total/1e6:.1f}M trainable parameters")

    def extract_embedding(self, x):
        patches, cls_tok, hw = self.backbone(x)
        parts = self.part_disc(patches, hw)
        # NEW: enrich parts with spatial relations
        rel_parts = self.part_rel(parts['part_features'], parts['part_positions'])
        emb = self.pool(rel_parts, parts['salience'])
...
# =============================================================================
# NEW: Relation Contrastive Loss
# =============================================================================
class RelationContrastiveLoss(nn.Module):
    """
    Aligns the inter-part relation structure between drone and satellite views.

    For each view, computes a K×K relation matrix R where R[i,j] = cos(part_i, part_j).
    Then applies row-wise softmax to get a soft relation distribution.
    Loss = symmetric KL divergence between drone and satellite relation distributions.

    This forces the model to preserve spatial layout patterns across views:
    if part 3 (road) is related to part 5 (building) in the drone view,
    the same relation should hold in the satellite view.
    """
    def __init__(self, temperature=0.1):
        super().__init__()
        self.temperature = temperature

    def forward(self, drone_parts, sat_parts):
        """
        drone_parts: [B, K, D] relation-enriched part features
        sat_parts:   [B, K, D] relation-enriched part features
        """
        # Compute relation matrices
        d_norm = F.normalize(drone_parts, dim=-1)
        s_norm = F.normalize(sat_parts, dim=-1)
        R_d = torch.bmm(d_norm, d_norm.transpose(1, 2)) / self.temperature  # [B, K, K]
        R_s = torch.bmm(s_norm, s_norm.transpose(1, 2)) / self.temperature  # [B, K, K]

        # Row-wise softmax → relation distributions
        P_d = F.softmax(R_d, dim=-1)  # [B, K, K]
        P_s = F.softmax(R_s, dim=-1)  # [B, K, K]

        # Symmetric KL divergence
        kl_ds = F.kl_div((P_d + 1e-8).log(), P_s, reduction='batchmean', log_target=False)
        kl_sd = F.kl_div((P_s + 1e-8).log(), P_d, reduction='batchmean', log_target=False)
...
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    proxy_anchor, ema_dist, relation_loss = new_losses
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # NEW: Relation Contrastive Loss on relation-enriched parts
            l_rel = relation_loss(d_out['rel_part_features'], s_out['rel_part_features'])
...
def main():
    ...
    print("=" * 65)
    print("  EXP31: SPDGeo-SPAR — Spatial Part Relation Transformer")
    print(f"  Base: SPDGeo-DPE (93.59% R@1)")
    print(f"  Novel: PartRelationTransformer({CFG.RELATION_LAYERS}L, {CFG.RELATION_HEADS}H) + RelationContrastiveLoss")
    print(f"  Dataset: SUES-200 | Epochs: {CFG.NUM_EPOCHS} | Device: {DEVICE}")
    print(f"  Losses: 8 DPE + 1 RelationContrastive = 9 total")
    print("=" * 65)
    ...
    print('\nBuilding models …')
    model = SPDGeoSPARModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    ...
    relation_loss = RelationContrastiveLoss(temperature=0.1)
    new_losses    = (proxy_anchor, ema_dist, relation_loss)
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"Proxy {ld['proxy']:.3f} EMA {ld['ema']:.3f} Rel {ld['rel']:.3f} | LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp31_spar_best.pth'))
    ...
    print(f'\n{"="*65}')
    print(f'  EXP31: SPDGeo-SPAR COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    for r in results_log:
        print(f'  Ep{r["epoch"]:3d} R@1:{r["R@1"]:6.2f} R@5:{r["R@5"]:6.2f} R@10:{r["R@10"]:6.2f} mAP:{r["mAP"]:6.2f}')

    with open(os.path.join(CFG.OUTPUT_DIR, 'exp31_spar_results.json'), 'w') as f:
