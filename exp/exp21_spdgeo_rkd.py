# =============================================================================
# EXP21: SPDGeo-RKD — Relational Knowledge Distillation
# =============================================================================
# Base:    SPDGeo-D (90.36% R@1) — DINOv2-S + DINOv2-B teacher + 7 losses
# Novel:   1) RKD-Distance — preserves pairwise distance structure from teacher
#             space to student space (Euclidean distance ratios)
#          2) RKD-Angle — preserves angular relations in sample triplets from
#             teacher to student (angle-preserving transfer)
#          3) Cross-View Structural Consistency — drone pairwise relations should
#             mirror satellite pairwise relations for same batch
#
# Motivation:
#   SPDGeo-D's CrossDistillationLoss aligns INDIVIDUAL features (student→teacher)
#   via MSE+Cosine. But this point-wise alignment ignores the STRUCTURE of the
#   teacher's representation space — how samples relate to each other.
#
#   Consider: If teacher places locations A,B close and C far, the student should
#   preserve this topology, not just minimize per-sample MSE. Relational KD
#   (Park et al., CVPR 2019) transfers these inter-sample relationships.
#
#   Additionally, the drone and satellite embeddings of the SAME batch should
#   have similar pairwise structure — if drone_i is close to drone_j, then
#   sat_i should be close to sat_j (Cross-View Structural Consistency).
#
# Key insight: RKD is complementary to point-wise distillation — it provides
#   a different supervision signal that preserves manifold structure. Combined
#   with the existing 7 losses, this adds structural inductive bias that
#   helps organize the learned embedding space more faithfully.
#
# Architecture: Identical to SPDGeo-D (no architecture change)
# Total losses: 10 (7 base + RKD-D + RKD-A + CVSC)
# =============================================================================
...
class Config:
    ...
    # NEW: Relational KD weights
    LAMBDA_RKD_D        = 0.3    # RKD-Distance loss
    LAMBDA_RKD_A        = 0.3    # RKD-Angle loss
    LAMBDA_CVSC         = 0.2    # Cross-View Structural Consistency
...
# =============================================================================
# STUDENT MODEL — SPDGeo-RKD (identical arch to SPDGeo-D)
# =============================================================================
class SPDGeoRKDModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        print(f"  SPDGeo-RKD student: {total/1e6:.1f}M trainable parameters")
...
# =============================================================================
# NEW LOSSES — Relational Knowledge Distillation
# =============================================================================
...
class RKDDistanceLoss(nn.Module):
    ...
    Transfers pairwise distance STRUCTURE from teacher to student.
    For each pair (i, j) in the batch, computes normalized pairwise distance
    in both teacher and student spaces, then minimizes their Huber loss.

    The normalization (dividing by mean distance) makes it scale-invariant —
    the student doesn't need to match absolute distances, only their ratios.

    This preserves the teacher's manifold topology: if teacher puts A,B close
    and C far, the student learns the same relative arrangement.
    """
    def forward(self, student_emb, teacher_emb):
        """
        student_emb: [B, D_s] student embeddings
        teacher_emb: [B, D_t] teacher embeddings (detached)
        """
        # Pairwise L2 distances
        with torch.no_grad():
            t_dist = torch.cdist(teacher_emb, teacher_emb, p=2)  # [B, B]
            t_mean = t_dist[t_dist > 0].mean().clamp(min=1e-6)
            t_dist_norm = t_dist / t_mean  # scale-invariant

        s_dist = torch.cdist(student_emb, student_emb, p=2)  # [B, B]
...
class RKDAngleLoss(nn.Module):
    """
    RKD-Angle (Park et al., CVPR 2019).

    For each triplet (i, j, k), computes the angle at j between vectors
    (i→j) and (k→j) in both teacher and student spaces.

    Preserves angular relations which capture higher-order structure
    beyond pairwise distances. Particularly useful for retrieval where
    relative ordering matters more than absolute distances.

    Implementation uses efficient vectorized angle computation:
    angle(i,j,k) = cosine( (e_i - e_j), (e_k - e_j) )
    """
    def forward(self, student_emb, teacher_emb):
        """
        student_emb: [B, D_s]
        teacher_emb: [B, D_t] (detached)
        """
        ...
        with torch.no_grad():
            t_diff = teacher_emb.unsqueeze(0) - teacher_emb.unsqueeze(1)  # [B, B, D]
            t_norm = F.normalize(t_diff, dim=-1)
            t_angle = torch.bmm(t_norm, t_norm.transpose(1, 2))  # [B, B, B]

        s_diff = student_emb.unsqueeze(0) - student_emb.unsqueeze(1)
        s_norm = F.normalize(s_diff, dim=-1)
        s_angle = torch.bmm(s_norm, s_norm.transpose(1, 2))  # [B, B, B]

        return F.smooth_l1_loss(s_angle, t_angle)


class CrossViewStructuralConsistencyLoss(nn.Module):
    """
    Cross-View Structural Consistency (CVSC).

    Novel loss specific to cross-view geo-localization:
    The pairwise similarity structure within drone embeddings should
    mirror the pairwise similarity structure within satellite embeddings.

    If drone_i is similar to drone_j (same neighborhood, same features),
    then sat_i should be similar to sat_j.

    This is different from RKD (which transfers teacher→student) —
    CVSC transfers structure between VIEWS of the SAME student.

    Loss = ||sim_matrix(drone) - sim_matrix(sat)||_F^2 / B^2
    """
    def forward(self, drone_emb, sat_emb):
        """
        drone_emb: [B, D] L2-normalized
        sat_emb:   [B, D] L2-normalized
        """
        drone_sim = drone_emb @ drone_emb.T  # [B, B]
        sat_sim   = sat_emb @ sat_emb.T      # [B, B]
        return F.mse_loss(drone_sim, sat_sim)
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            if teacher is not None:
                ...
                # === NEW: RKD from teacher ===
                l_rkd_d = 0.5 * (rkd_d(d_out['embedding'], t_drone) +
                                  rkd_d(s_out['embedding'], t_sat))
                l_rkd_a = 0.5 * (rkd_a(d_out['embedding'], t_drone) +
                                  rkd_a(s_out['embedding'], t_sat))
            else:
                l_cross = torch.tensor(0.0, device=device)
                l_rkd_d = torch.tensor(0.0, device=device)
                l_rkd_a = torch.tensor(0.0, device=device)
            ...
            # === NEW: Cross-View Structural Consistency ===
            l_cvsc = cvsc(d_out['embedding'], s_out['embedding'])
        ...
        loss_sums['rkd_d'] += l_rkd_d.item() if torch.is_tensor(l_rkd_d) else l_rkd_d
        loss_sums['rkd_a'] += l_rkd_a.item() if torch.is_tensor(l_rkd_a) else l_rkd_a
        loss_sums['cvsc']  += l_cvsc.item()
...
def main():
    ...
    print("=" * 65)
    print("  EXP21: SPDGeo-RKD — Relational Knowledge Distillation")
    print(f"  Base: SPDGeo-D (90.36% R@1)")
    print(f"  Novel: RKD-Distance + RKD-Angle + Cross-View Structural Consistency")
    ...
    print(f"  Losses: 7 base + 3 new = 10 total")
    print("=" * 65)
    ...
    print('\nBuilding models …')
    model = SPDGeoRKDModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    try:
    ...
    # New losses
    rkd_d = RKDDistanceLoss()
    rkd_a = RKDAngleLoss()
    cvsc  = CrossViewStructuralConsistencyLoss()
    new_losses = (rkd_d, rkd_a, cvsc)
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"UAPA {ld['uapa']:.3f} | "
              f"RKD-D {ld['rkd_d']:.3f}  RKD-A {ld['rkd_a']:.3f}  CVSC {ld['cvsc']:.3f} | "
              f"LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp21_rkd_best.pth'))
                print(f"  ★ New best R@1: {best_r1:.2f}%!")

    print(f'\n{"="*65}')
    print(f'  EXP21: SPDGeo-RKD COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp21_rkd_results.json'), 'w') as f:
