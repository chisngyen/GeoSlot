# =============================================================================
# EXP24: SPDGeo-OTML — Optimal Transport Metric Learning
# =============================================================================
# Base:    SPDGeo-D (90.36% R@1) — DINOv2-S + DINOv2-B teacher + 7 losses
# Novel:   1) Sinkhorn-based Part-to-Part OT Matching — instead of forcing
#             part_k(drone) == part_k(sat) by index, learn optimal soft
#             assignment between drone parts and satellite parts via OT
#          2) Earth Mover's Distance (EMD) Loss — use Sinkhorn-approximated
#             Wasserstein distance between part distributions as a
#             geometrically meaningful cross-view similarity metric
#          3) OT-Guided Contrastive — use OT plans to weight part-level
#             contrastive matching (parts with high OT mass = important)
#
# Motivation:
#   SPDGeo-D assumes part i in drone = part i in satellite (shared prototypes).
#   This is reasonable for SIMILAR viewpoints but breaks down when:
#   - Drone rotates → part layout shifts relative to satellite
#   - Different FOV/altitude → different parts visible
#   - Occluded/ambiguous regions → different part assignments per view
#
#   Optimal Transport finds the MINIMUM-COST assignment between two
#   distributions — it can discover that drone_part_3 actually corresponds
#   to sat_part_5 in this particular pair, not sat_part_3.
#
#   The Sinkhorn algorithm (Cuturi, NeurIPS 2013) provides a differentiable,
#   GPU-efficient approximation of OT. Recent works (SuperGlue, LoFTR)
#   have shown OT-based matching is SOTA for keypoint correspondence.
#
#   We apply OT at the PART level (K=8 parts), not patch level — this is
#   much cheaper than pixel-level OT while still capturing viewpoint changes.
#
# Key insight: OT matching is complementary to shared-prototype matching.
#   Shared prototypes handle the COMMON case (parts align by index),
#   OT handles the HARD case (parts misalign due to viewpoint change).
#
# Architecture: Identical to SPDGeo-D + OT matching head
# Total losses: 9 (7 base + OT-EMD + OT-Contrastive)
# =============================================================================
...
class Config:
    ...
    # NEW: OT parameters
    SINKHORN_ITERS      = 5       # Sinkhorn iterations (5 is enough for K=8)
    SINKHORN_REG        = 0.1     # entropic regularization (ε)
    LAMBDA_OT_EMD       = 0.3     # OT-EMD loss weight
    LAMBDA_OT_CONTRAST  = 0.2     # OT-Guided contrastive loss weight
...
# =============================================================================
# STUDENT MODEL — SPDGeo-OTML (identical arch + OT head)
# =============================================================================
class SPDGeoOTMLModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        # OT cost projection: project parts to shared space for cost computation
        self.ot_proj = nn.Sequential(
            nn.Linear(cfg.PART_DIM, cfg.PART_DIM),
            nn.LayerNorm(cfg.PART_DIM)
        )

        total = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"  SPDGeo-OTML student: {total/1e6:.1f}M trainable parameters")
    ...
    def forward(self, x, return_parts=False):
        ...
        # Project part features for OT cost
        ot_feats = self.ot_proj(parts['part_features'])  # [B, K, D]

        out = {'embedding': fused, 'logits': logits, 'cls_logits': cls_logits,
               'projected_feat': projected_feat, 'ot_feats': ot_feats}
...
# =============================================================================
# NEW: Sinkhorn OT Matching Losses
# =============================================================================

def sinkhorn_transport(cost, n_iters=5, reg=0.1):
    """
    Sinkhorn-Knopp algorithm for optimal transport (Cuturi, NeurIPS 2013).

    Given a cost matrix C ∈ R^{K×K}, find the optimal transport plan T
    that minimizes <C, T> + ε * H(T), subject to T1=μ, T^T1=ν
    where μ = ν = uniform(1/K).

    The entropic regularization makes OT differentiable and efficient.

    Args:
        cost:    [B, K, K] pairwise cost between drone parts and sat parts
        n_iters: number of Sinkhorn iterations
        reg:     entropic regularization ε

    Returns:
        transport: [B, K, K] soft assignment matrix (rows/cols sum to 1/K)
    """
    B, K, _ = cost.shape
    # Log-domain Sinkhorn for numerical stability
    log_K = -cost / reg  # [B, K, K]

    # Uniform marginals
    log_mu = torch.full((B, K), -math.log(K), device=cost.device)
    log_nu = torch.full((B, K), -math.log(K), device=cost.device)

    u = torch.zeros_like(log_mu)  # [B, K]
    v = torch.zeros_like(log_nu)  # [B, K]

    for _ in range(n_iters):
        u = log_mu - torch.logsumexp(log_K + v.unsqueeze(1), dim=2)  # [B, K]
        v = log_nu - torch.logsumexp(log_K + u.unsqueeze(2), dim=1)  # [B, K]

    transport = (log_K + u.unsqueeze(2) + v.unsqueeze(1)).exp()  # [B, K, K]
    return transport


class OTEMDLoss(nn.Module):
    """
    Earth Mover's Distance (EMD) Loss via Sinkhorn OT.

    For positive pairs (same location), minimizes the Wasserstein distance
    between drone and satellite part distributions → parts should be
    matchable across views.

    For negative pairs, maximizes the Wasserstein distance → parts from
    different locations should be far apart in transport cost.

    EMD = <C, T*> where T* is the optimal transport plan and C is the cost.
    """
    def __init__(self, n_iters=5, reg=0.1, margin=0.5):
        super().__init__()
        self.n_iters = n_iters
        self.reg = reg
        self.margin = margin

    def forward(self, drone_parts, sat_parts, labels):
        """
        drone_parts: [B, K, D] OT-projected part features
        sat_parts:   [B, K, D] OT-projected part features
        labels:      [B] class labels
        """
        B, K, D = drone_parts.shape

        # Normalize for cosine cost
        d_norm = F.normalize(drone_parts, dim=-1)
        s_norm = F.normalize(sat_parts, dim=-1)

        # Pairwise cost: cosine distance between all part pairs
        # cost[b, i, j] = 1 - cos(drone_part_i, sat_part_j)
        cost = 1.0 - torch.bmm(d_norm, s_norm.transpose(1, 2))  # [B, K, K]

        # Compute OT transport plan
        transport = sinkhorn_transport(cost, self.n_iters, self.reg)  # [B, K, K]

        # EMD: <C, T> = sum of element-wise product
        emd = (cost * transport).sum(dim=(1, 2))  # [B]

        # For matched pairs: minimize EMD
        # For non-matched: maximize EMD (push apart)
        labels_eq = labels.view(-1, 1).eq(labels.view(1, -1)).float()

        # Average positive EMD (should be small)
        pos_emd = emd.mean()

        return pos_emd


class OTGuidedContrastiveLoss(nn.Module):
    """
    OT-Guided Contrastive Loss.

    Uses the OT transport plan to create part-level similarity scores,
    then uses these as a more nuanced similarity metric for contrastive
    learning (instead of just global embedding dot products).

    OT similarity = -EMD(drone_i, sat_j)  (lower cost = more similar)

    This provides a geometrically meaningful similarity that accounts
    for part correspondence, not just global feature overlap.
    """
    def __init__(self, n_iters=5, reg=0.1, temp=0.1):
        super().__init__()
        self.n_iters = n_iters
        self.reg = reg
        self.temp = temp
    ...
    def forward(self, drone_parts, sat_parts, labels):
        ...
        # Compute all-pairs OT similarity
        ot_sim = torch.zeros(B, B, device=drone_parts.device)

        for i in range(B):
            # Cost between drone_i parts and all sat parts
            d_i = d_norm[i:i+1].expand(B, -1, -1)  # [B, K, D]
            cost = 1.0 - torch.bmm(d_i, s_norm.transpose(1, 2))  # [B, K, K]
            transport = sinkhorn_transport(cost, self.n_iters, self.reg)
            emd = (cost * transport).sum(dim=(1, 2))  # [B]
            ot_sim[i] = -emd  # higher = more similar

        # InfoNCE-style loss using OT similarity
        ot_sim = ot_sim / self.temp
        pos_mask = labels.view(-1, 1).eq(labels.view(1, -1)).float()
        log_prob = ot_sim - torch.logsumexp(ot_sim, dim=1, keepdim=True)
        return (-(log_prob * pos_mask).sum(1) / pos_mask.sum(1).clamp(min=1)).mean()
...
# TRAINING
# =============================================================================
def train_one_epoch(model, teacher, loader, losses, new_losses, optimizer,
                    scaler, device, epoch):
    model.train()
    if teacher: teacher.eval()

    infonce, triplet, ce, consist, cross_dist, self_dist, uapa = losses
    ot_emd, ot_contrast = new_losses
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # === NEW: OT Losses ===
            # OT-EMD: minimize transport cost between matched drone/sat parts
            l_ot_emd = ot_emd(d_out['ot_feats'], s_out['ot_feats'], labels)

            # OT-Guided Contrastive: use OT similarity for contrastive learning
            l_ot_con = ot_contrast(d_out['ot_feats'], s_out['ot_feats'], labels)
            ...
            loss = (CFG.LAMBDA_CE          * l_ce +
                    ...
                    CFG.LAMBDA_OT_EMD      * l_ot_emd +
                    CFG.LAMBDA_OT_CONTRAST * l_ot_con)
        ...
        loss_sums['ot_emd'] += l_ot_emd.item()
        loss_sums['ot_con'] += l_ot_con.item()
...
def main():
    ...
    print("=" * 65)
    print("  EXP24: SPDGeo-OTML — Optimal Transport Metric Learning")
    print(f"  Base: SPDGeo-D (90.36% R@1)")
    print(f"  Novel: Sinkhorn OT Part Matching + EMD Loss + OT-Guided Contrastive")
    print(f"  Sinkhorn: {CFG.SINKHORN_ITERS} iters, ε={CFG.SINKHORN_REG}")
    ...
    print(f"  Losses: 7 base + OT-EMD + OT-Contrastive = 9 total")
    print("=" * 65)
    ...
    print('\nBuilding models …')
    model = SPDGeoOTMLModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    try:
    ...
    # New losses
    ot_emd      = OTEMDLoss(n_iters=CFG.SINKHORN_ITERS, reg=CFG.SINKHORN_REG)
    ot_contrast = OTGuidedContrastiveLoss(n_iters=CFG.SINKHORN_ITERS, reg=CFG.SINKHORN_REG)
    new_losses  = (ot_emd, ot_contrast)
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"UAPA {ld['uapa']:.3f} | "
              f"OT-EMD {ld['ot_emd']:.3f}  OT-Con {ld['ot_con']:.3f} | "
              f"LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp24_otml_best.pth'))
                print(f"  ★ New best R@1: {best_r1:.2f}%!")

    print(f'\n{"="*65}')
    print(f'  EXP24: SPDGeo-OTML COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp24_otml_results.json'), 'w') as f:
