"""
EXP8: GeoEVA — EVA02-Large Teacher + Intermediate Feature Distillation
=======================================================================
Teacher:  EVA02-Large (304M params, 1024-dim) via timm
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) EVA02-Large teacher (CLIP-aligned features!)
          2) Intermediate Feature Distillation — distill from teacher intermediate layers
          3) Feature Covariance Alignment — match feature correlation structure
Expected: 80-84% R@1
"""
...
class Config:
    ...
    LAMBDA_IFD=0.2   # Intermediate Feature Distillation
    LAMBDA_FCA=0.1   # Feature Covariance Alignment
    MARGIN=0.3
...
class ConvNeXtBlock(nn.Module):
    def __init__(self, dim, dpr=0., lsi=1e-6):
        super().__init__(); self.dwconv=nn.Conv2d(dim,dim,7,padding=3,groups=dim)
        self.norm=LayerNorm(dim); self.pwconv1=nn.Linear(dim,4*dim); self.act=nn.GELU()
...
class EVA02Teacher(nn.Module):
    """EVA02-Large Teacher — CLIP-aligned ViT-Large (304M params, 1024-dim)"""
    def __init__(self, ntb=2):
        super().__init__()
        print("Loading EVA02-Large teacher via timm...")
        ...
        if self.model is None:
            raise RuntimeError("Could not load any EVA02 model")
        ...
        print(f"  EVA02-Large: {t/1e6:.0f}M total, {tr/1e6:.1f}M trainable")

    @torch.no_grad()
    def forward(self, x):
        return self.model(x)  # [B, 1024]

    def forward_intermediate(self, x, layers=[6, 12, 18, 23]):
        """Extract features from intermediate layers for multi-layer distillation"""
        with torch.no_grad():
            ...
            for i, blk in enumerate(self.model.blocks):
                x = blk(x)
                if i in layers:
                    intermediates.append(x[:, 0])  # CLS token at each layer

            if hasattr(self.model, 'norm'):
                x = self.model.norm(x)
            intermediates.append(x[:, 0])  # Final CLS
...
# === NEW: Feature Covariance Alignment ===
class FeatureCovarianceAlignment(nn.Module):
    """Match covariance structure of student features to teacher features.
    The covariance matrix captures feature correlations — matching it
    transfers structural knowledge beyond individual feature values."""
    def __init__(self, student_dim=768, teacher_dim=1024):
        super().__init__()
        self.proj = nn.Linear(student_dim, teacher_dim) if student_dim != teacher_dim else nn.Identity()

    def forward(self, student_feat, teacher_feat):
        s = self.proj(student_feat)  # [B, D]
        t = teacher_feat

        # Center features
        s_centered = s - s.mean(0, keepdim=True)
        t_centered = t - t.mean(0, keepdim=True)

        B = s.size(0)
        # Compute covariance matrices (use subset of dims for memory)
        D = min(s.size(1), 256)
        s_cov = (s_centered[:, :D].T @ s_centered[:, :D]) / (B - 1)
        with torch.no_grad():
            t_cov = (t_centered[:, :D].T @ t_centered[:, :D]) / (B - 1)
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.cd=CrossDistillationLoss(768,1024)
        self.fca=FeatureCovarianceAlignment(768,1024)  # NEW
    def forward(self,do,so,l,td=None,ts=None):
        ...
        if td is not None:
            ...
            losses['fca']=self.cfg.LAMBDA_FCA*(self.fca(do['final_feature'],td)+self.fca(so['final_feature'],ts))
...
def main():
    ...
    print("="*60+"\nEXP8: GeoEVA — EVA02-Large + Feature Covariance Alignment\n"+"="*60)
    ...
    try: teacher=EVA02Teacher(2).to(dev)
    except Exception as e: print(f"EVA02 failed:{e}"); teacher=None
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp8.pth')); print("  ★ Best!")
