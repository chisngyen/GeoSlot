"""
EXP17: GeoBarlow — Barlow Twins Redundancy Reduction + MI Maximization
========================================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) Barlow Twins Loss — reduce feature redundancy via cross-correlation
          2) MINE (Mutual Info Neural Estimation) — maximize MI between drone-satellite
          3) Feature Whitening — decorrelate feature dimensions
Ref:      Barlow Twins (ICML 2021) + MINE (ICLR 2018), novel combination for geolocalization
Expected: 82-86% R@1 (strong theoretical foundation, no arch changes)
"""
...
class Config:
    ...
    LAMBDA_BARLOW=0.25     # Barlow Twins
    LAMBDA_MINE=0.15       # Mutual Info
    BARLOW_LAMBDA_OFF=0.005  # Off-diagonal weight in Barlow Twins
...
def get_transforms(mode="train"):
    ...
    return T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),T.ToTensor(),T.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
...
# ============================================================================
# NEW: Barlow Twins Cross-View Loss (ICML 2021 inspired)
# ============================================================================
class BarlowTwinsLoss(nn.Module):
    """Cross-correlation between drone and satellite features should be identity.
    Diagonal = 1 (invariance): drone and sat features of same location should match.
    Off-diagonal = 0 (redundancy reduction): different dimensions should be uncorrelated.

    This prevents dimensional collapse where all info is in few dims.
    Original: Barlow Twins (ICML 2021) — we adapt for CROSS-VIEW alignment."""
    def __init__(self, dim=768, lambda_off=0.005):
        super().__init__()
    ...
    def forward(self, drone_feat, sat_feat):
        ...
        z_d = self.bn(drone_feat)  # [B, D]
        z_s = self.bn(sat_feat)    # [B, D]
        ...
        loss_inv = (1 - diag).pow(2).sum()  # Invariance term

        # Zero out diagonal for off-diagonal computation
        off_diag = c.pow(2)
        off_diag[range(D), range(D)] = 0
        loss_red = off_diag.sum() * self.lambda_off  # Redundancy reduction

        return loss_inv + loss_red


# ============================================================================
# NEW: MINE — Mutual Information Neural Estimation (ICLR 2018)
# ============================================================================
class MINELoss(nn.Module):
    """Maximize mutual information between drone and satellite embeddings.
    Uses a small neural network (statistics network) to estimate MI.
    MI captures non-linear dependencies beyond what contrastive loss captures.

    I(X;Y) >= E_joint[T(x,y)] - log(E_marginal[e^T(x,y)])"""
    ...
    def forward(self, drone_feat, sat_feat):
        B = drone_feat.size(0)
        if B < 4: return torch.tensor(0., device=drone_feat.device)

        # Joint distribution: (drone_i, sat_i) - same location pairs
        joint = torch.cat([drone_feat, sat_feat], dim=1)  # [B, 2D]
        t_joint = self.stats_net(joint)  # [B, 1]

        # Marginal: shuffle satellite to break correspondence
        perm = torch.randperm(B, device=sat_feat.device)
        marginal = torch.cat([drone_feat, sat_feat[perm]], dim=1)
        t_marginal = self.stats_net(marginal)

        # MINE objective: maximize MI (so minimize negative MI)
        mi_estimate = t_joint.mean() - torch.log(torch.exp(t_marginal).mean() + 1e-8)
        return -mi_estimate  # Negate because we minimize loss
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.barlow=BarlowTwinsLoss(cfg.EMBED_DIM, cfg.BARLOW_LAMBDA_OFF)
        self.mine=MINELoss(cfg.EMBED_DIM)
    def forward(self,do,so,l,td=None,ts=None):
        ...
        # Barlow Twins: cross-view redundancy reduction
        losses['barlow']=self.cfg.LAMBDA_BARLOW*self.barlow(do['embedding'],so['embedding'])
        # MINE: maximize MI between drone and satellite
        losses['mine']=self.cfg.LAMBDA_MINE*self.mine(do['embedding'],so['embedding'])
...
def main():
    ...
    print("="*60+"\nEXP17: GeoBarlow — Barlow Twins + MI Maximization\n"+"="*60)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp17.pth')); print("  ★ Best!")
