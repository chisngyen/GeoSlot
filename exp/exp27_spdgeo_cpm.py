# =============================================================================
# EXP27: SPDGeo-CPM — Curriculum Proxy with Progressive Margin
# =============================================================================
# Base:    SPDGeo-DPE (93.59% R@1) — THE CHAMPION
# Novel:   Addresses DPE's ep40 saturation via 3 curriculum mechanisms:
#          1) Progressive Margin Schedule — δ: 0.05→0.25 cosine ramp-up
#             Early: small margin = easy convergence; Late: large margin = push
#          2) Proxy Perturbation — Gaussian noise σ₀×(1-t/T) on proxies
#             Early: high noise = harder proxy matching; Late: stabilizes
#          3) Hard Sample Reweighting — w_i = 1 + β×(1-cos_sim)
#             Hard-negative pairs get higher weight throughout training
#
# Motivation:
#   DPE peaks at ep40 then regresses 2.3% by ep120. This suggests:
#   - ProxyAnchor margin=0.1 is too easy → proxies converge too fast
#   - The fixed difficulty makes ep40+ training redundant
#   - Solution: curriculum learning — increase difficulty over epochs
#
#   Curriculum Proxy Mining (CPM) is a principled approach to prevent
#   premature convergence in metric learning with learnable proxies.
#
# Architecture: SPDGeo-DPE + CurriculumProxyAnchor + HardSampleReweighting
# Total losses: 8 (same structure as DPE, with curriculum modifications)
# =============================================================================
...
class Config:
    ...
    # NEW: Curriculum hyperparams
    MARGIN_START        = 0.05   # Easy start margin
    MARGIN_END          = 0.25   # Hard final margin
    PERTURB_SIGMA_START = 0.05   # Initial proxy perturbation σ
...
# STUDENT MODEL
# =============================================================================
class SPDGeoCPMModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        print(f"  SPDGeo-CPM student: {total/1e6:.1f}M trainable parameters")
...
# =============================================================================
# NEW: Curriculum ProxyAnchor Loss
# =============================================================================
class CurriculumProxyAnchorLoss(nn.Module):
    """
    ProxyAnchor with three curriculum mechanisms to prevent ep40 saturation:

    1) Progressive Margin: δ(t) = δ_start + (δ_end - δ_start) × (1 - cos(πt/T))/2
       Cosine ramp from easy (0.05) to hard (0.25) margin over training.

    2) Proxy Perturbation: Add Gaussian noise N(0, σ(t)²) to proxies during training
       σ(t) = σ₀ × (1 - t/T) — high noise early (harder matching), decays to 0.
       This prevents proxies from converging too quickly to cluster centers.

    3) Hard Sample Reweighting: w_i = 1 + β × (1 - cos_sim(x_i, p_yi))
       Samples far from their proxy get higher weight, maintaining gradient for
       hard samples even when easy samples are well-clustered.

    These three mechanisms work together: Progressive Margin increases geometric
    difficulty, Perturbation maintains proxy diversity, and Reweighting focuses
    compute on the informative tail.
    """
    def __init__(self, num_classes, embed_dim, alpha=32,
                 margin_start=0.05, margin_end=0.25,
                 perturb_sigma=0.05, hard_beta=0.5):
        super().__init__()
    ...
    def get_margin(self, epoch, total_epochs):
        """Cosine ramp-up from margin_start to margin_end."""
        progress = min(epoch / max(total_epochs, 1), 1.0)
        return self.margin_start + (self.margin_end - self.margin_start) * 0.5 * (1 - math.cos(math.pi * progress))

    def get_perturb_sigma(self, epoch, total_epochs):
        """Linear decay from sigma_start to 0."""
        progress = min(epoch / max(total_epochs, 1), 1.0)
        return self.perturb_sigma * (1 - progress)

    def forward(self, embeddings, labels, epoch=1, total_epochs=120):
        """
        embeddings: [B, D] L2-normalized
        labels:     [B] class indices
        epoch:      current training epoch
        total_epochs: total epochs for scheduling
        """
        margin = self.get_margin(epoch, total_epochs)
        sigma = self.get_perturb_sigma(epoch, total_epochs)

        P = F.normalize(self.proxies, dim=-1)

        # Proxy perturbation: add decaying noise during training
        if self.training and sigma > 1e-6:
            noise = torch.randn_like(P) * sigma
            P_perturbed = F.normalize(P + noise, dim=-1)
        else:
            P_perturbed = P

        sim = embeddings @ P_perturbed.T  # [B, C]
        one_hot = F.one_hot(labels, self.num_classes).float()

        # Hard sample reweighting: higher weight for samples far from their proxy
        with torch.no_grad():
            pos_sim_values = (sim * one_hot).sum(dim=-1)  # [B]
            weights = 1.0 + self.hard_beta * (1.0 - pos_sim_values)  # [B]
            weights = weights.unsqueeze(-1)  # [B, 1]

        # Positive term with progressive margin and reweighting
        pos_exp = torch.exp(-self.alpha * (sim * one_hot - margin))
        pos_exp = pos_exp * one_hot * weights  # Apply sample weights
        ...
        # Negative term with progressive margin
        neg_mask = 1 - one_hot
        neg_exp = torch.exp(self.alpha * (sim * neg_mask + margin))
...
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    curriculum_proxy, ema_dist = new_losses

    total_sum = 0; n = 0; loss_sums = defaultdict(float)
    cur_margin = curriculum_proxy.get_margin(epoch, CFG.NUM_EPOCHS)
    cur_sigma = curriculum_proxy.get_perturb_sigma(epoch, CFG.NUM_EPOCHS)

    for batch in tqdm(loader, desc=f'Ep{epoch:3d} δ={cur_margin:.3f} σ={cur_sigma:.4f}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # Curriculum ProxyAnchor (epoch-dependent margin, perturbation, reweighting)
            l_proxy = 0.5 * (curriculum_proxy(d_out['embedding'], labels, epoch, CFG.NUM_EPOCHS) +
                             curriculum_proxy(s_out['embedding'], labels, epoch, CFG.NUM_EPOCHS))
...
def main():
    ...
    print("=" * 65)
    print("  EXP27: SPDGeo-CPM — Curriculum Proxy with Progressive Margin")
    print(f"  Base: SPDGeo-DPE (93.59% R@1)")
    print(f"  Novel: ProgressiveMargin δ:{CFG.MARGIN_START}→{CFG.MARGIN_END} + "
          f"ProxyPerturb σ₀={CFG.PERTURB_SIGMA_START} + "
          f"HardReweight β={CFG.HARD_BETA}")
    ...
    print(f"  Losses: 6 base + CurriculumProxy + EMADist = 8 total")
    print("=" * 65)
    ...
    print('\nBuilding models …')
    model = SPDGeoCPMModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    try:
    ...
    # New: Curriculum proxy + EMA dist
    curriculum_proxy = CurriculumProxyAnchorLoss(
        CFG.NUM_CLASSES, CFG.EMBED_DIM, alpha=CFG.PROXY_ALPHA,
        margin_start=CFG.MARGIN_START, margin_end=CFG.MARGIN_END,
        perturb_sigma=CFG.PERTURB_SIGMA_START, hard_beta=CFG.HARD_BETA
    ).to(DEVICE)
    ema_dist = EMADistillationLoss()
    new_losses = (curriculum_proxy, ema_dist)
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': curriculum_proxy.parameters(),  'lr': CFG.LR},
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"δ={cur_margin:.3f} σ={cur_sigma:.4f} LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp27_cpm_best.pth'))
    ...
    print(f'\n{"="*65}')
    print(f'  EXP27: SPDGeo-CPM COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp27_cpm_results.json'), 'w') as f:
