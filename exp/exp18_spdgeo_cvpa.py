# =============================================================================
# EXP18: SPDGeo-CVPA — Cross-View Part Alignment
# =============================================================================
# Base:    SPDGeo-D (90.36% R@1) — DINOv2-S + DINOv2-B teacher + 7 losses
# Novel:   1) Part Alignment Loss — cosine alignment of same-index parts across views
#          2) Part-Level InfoNCE — contrastive at individual part granularity
#          3) Part Diversity Regularizer — maximize inter-prototype distance
# Expected: 91-93% R@1 (finer-grained cross-view correspondence)
#
# Motivation:
#   SPDGeo-D discovers semantic parts independently per view using shared prototypes.
#   While global contrastive losses (SupInfoNCE, Triplet) align OVERALL embeddings,
#   they don't enforce fine-grained part-level correspondence. Part k in a drone image
#   should produce similar features to Part k in the satellite image of the same location.
#   This experiment adds part-level supervision to exploit the shared prototype structure.
#
# Key insight: Since both views use the SAME prototypes, Part k in drone corresponds
#   to Part k in satellite. We add losses to:
#   (a) Align matched parts directly (PartAlignmentLoss)
#   (b) Contrast parts at individual granularity (PartLevelContrastiveLoss)
#   (c) Ensure prototypes stay diverse (PartDiversityLoss)
#   These are TRAINING-ONLY regularizations — inference is unchanged from SPDGeo-D.
#
# Architecture: Identical to SPDGeo-D + 3 new auxiliary losses
# Total losses: 10 (7 from SPDGeo-D + 3 new)
# =============================================================================
...
class Config:
    ...
    # NEW: Part-level alignment loss weights
    LAMBDA_PART_ALIGN   = 0.3   # direct part cosine alignment
    LAMBDA_PART_NCE     = 0.2   # part-level InfoNCE
    LAMBDA_PART_DIV     = 0.05  # prototype diversity

    PART_NCE_TEMP       = 0.07  # temperature for part-level InfoNCE
    DISTILL_TEMP        = 4.0
...
# =============================================================================
# STUDENT MODEL — SPDGeo-CVPA
# =============================================================================
class SPDGeoCVPAModel(nn.Module):
    """
    SPDGeo-D + Cross-View Part Alignment.
    Model architecture is IDENTICAL to SPDGeo-D.
    The new part-level losses operate on the parts dict output.
    """
    def __init__(self, num_classes, cfg=CFG):
        ...
        print(f"  SPDGeo-CVPA student: {total/1e6:.1f}M trainable parameters")
...
# =============================================================================
# NEW LOSSES — Cross-View Part Alignment
# =============================================================================

class PartAlignmentLoss(nn.Module):
    """
    Direct cosine alignment of same-index parts across drone and satellite views.
    Since both views use the same semantic prototypes, Part k in drone corresponds
    to Part k in satellite — they represent the same semantic concept.

    Loss = mean(1 - cos_sim(drone_part_k, sat_part_k)) over all k and batch.
    """
    def forward(self, drone_parts, sat_parts):
        # drone_parts, sat_parts: [B, K, D]
        d = F.normalize(drone_parts, dim=-1)
        s = F.normalize(sat_parts, dim=-1)
        cos_sim = (d * s).sum(-1)  # [B, K]
        return (1 - cos_sim).mean()


class PartLevelContrastiveLoss(nn.Module):
    """
    InfoNCE at individual part granularity.

    For each part index k, treats drone part features as queries and satellite
    part features as keys. Same-location → positive, different-location → negative.
    Provides K independent contrastive signals, one per semantic concept.

    This addresses triplet saturation: even when global embeddings are well-separated,
    individual parts may still benefit from finer-grained contrastive supervision.
    """
    ...
    def forward(self, drone_parts, sat_parts, labels):
        """
        drone_parts: [B, K, D], sat_parts: [B, K, D], labels: [B]
        """
        B, K, D = drone_parts.shape
        lbl = labels.view(-1, 1)
        pos_mask = lbl.eq(lbl.T).float()  # [B, B]
        loss = 0.0

        for k in range(K):
            dq = F.normalize(drone_parts[:, k], dim=-1)  # [B, D]
            sr = F.normalize(sat_parts[:, k], dim=-1)     # [B, D]
            sim = dq @ sr.T / self.temp                    # [B, B]
            log_prob = sim - torch.logsumexp(sim, dim=1, keepdim=True)
            part_loss = -(log_prob * pos_mask).sum(1) / pos_mask.sum(1).clamp(min=1)
            loss += part_loss.mean()

        return loss / K


class PartDiversityLoss(nn.Module):
    """
    Encourages semantic prototypes to be maximally diverse (spread in feature space).
    Penalizes high cosine similarity between different prototypes.
    Prevents part collapse where multiple prototypes discover the same concept.

    Loss = mean(|cos_sim(p_i, p_j)|) for all i ≠ j.
    """
    def forward(self, prototypes):
        # prototypes: [K, D] — the learnable prototype parameters
...
# TRAINING
# =============================================================================
def train_one_epoch(model, teacher, loader, losses, new_losses, optimizer, scaler,
                    device, epoch):
    model.train()
    if teacher: teacher.eval()

    infonce, triplet, ce, consist, cross_dist, self_dist, uapa = losses
    part_align, part_nce, part_div = new_losses
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # === NEW: Part-level alignment losses (3 new) ===
            drone_pf = d_out['parts']['part_features']  # [B, K, D]
            sat_pf   = s_out['parts']['part_features']  # [B, K, D]

            l_part_align = part_align(drone_pf, sat_pf)
            l_part_nce   = part_nce(drone_pf, sat_pf, labels)
            l_part_div   = part_div(model.part_disc.prototypes)
            ...
            loss = (CFG.LAMBDA_CE          * l_ce +
                    ...
                    CFG.LAMBDA_PART_ALIGN  * l_part_align +
                    CFG.LAMBDA_PART_NCE    * l_part_nce +
                    CFG.LAMBDA_PART_DIV    * l_part_div)
        ...
        loss_sums['p_align'] += l_part_align.item()
        loss_sums['p_nce']   += l_part_nce.item()
        loss_sums['p_div']   += l_part_div.item()
...
def main():
    ...
    print("=" * 65)
    print("  EXP18: SPDGeo-CVPA — Cross-View Part Alignment")
    print(f"  Base: SPDGeo-D (90.36% R@1)")
    print(f"  Novel: PartAlignmentLoss + PartLevelInfoNCE + PartDiversityLoss")
    ...
    print('\nBuilding models …')
    model = SPDGeoCVPAModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    try:
    ...
    # New losses
    part_align = PartAlignmentLoss()
    part_nce   = PartLevelContrastiveLoss(temperature=CFG.PART_NCE_TEMP)
    part_div   = PartDiversityLoss()
    new_losses = (part_align, part_nce, part_div)
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"UAPA {ld['uapa']:.3f} | "
              f"PAlign {ld['p_align']:.3f}  PNCE {ld['p_nce']:.3f}  PDiv {ld['p_div']:.3f} | "
              f"LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if metrics['R@1'] > best_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp18_cvpa_best.pth'))
                print(f"  ★ New best R@1: {best_r1:.2f}%!")

    print(f'\n{"="*65}')
    print(f'  EXP18: SPDGeo-CVPA COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp18_cvpa_results.json'), 'w') as f:
