# =============================================================================
# EXP28: SPDGeo-AHN — Altitude-Hardness-Aware Negative Mining
# =============================================================================
# Base:    SPDGeo-DPE (93.59% R@1) — THE CHAMPION
# Novel:   Targets the persistent 150m altitude gap (6.92% below 300m in DPE):
#          1) AltitudeStratifiedSampler — guarantees 25% of each batch is 150m
#             (default PK sampler randomly under-samples 150m which has
#              the hardest viewpoint differences)
#          2) AltitudeWeightedProxyAnchor — λ_alt = 1/R@1_running per altitude
#             Poor-performing altitudes automatically get higher loss weight.
#             Running R@1 estimated via proxy similarity as a surrogate.
#          3) CrossAltitudeHardPair — within same location, forces drone-sat
#             pairs across extreme altitudes (150m drone + same-loc sat)
#             to be explicitly contrasted as positives, boosting hard pairs.
#
# Motivation:
#   Across all experiments, the 150m→300m R@1 gap persists at 4-7%.
#   The root cause is two-fold:
#   (a) At 150m, objects are larger but context is narrower → unique
#       landmarks dominate, but overlapping FOV with satellite is minimal
#   (b) Standard PK sampling treats each altitude equally by random chance,
#       but 150m images need MORE exposure because they're inherently harder
#
#   AHN directly addresses both: guaranteed 150m exposure (sampler) +
#   adaptive weighting (loss) + explicit hard-altitude positive pairs.
#
# Architecture: SPDGeo-DPE + AltitudeStratifiedSampler + AltWeightedProxy
# Total losses: 9 (6 base + AltWeightedProxy + EMA + CrossAltPair)
# =============================================================================
...
class Config:
    ...
    # NEW: Cross-altitude hard pair loss
    LAMBDA_CROSS_ALT    = 0.3

    # NEW: Altitude weight EMA smoothing
...
# =============================================================================
# DATASET — with altitude tracking
# =============================================================================
class SUES200Dataset(Dataset):
    def __init__(self, root, mode="train", altitudes=None, transform=None):
        ...
        self.drone_by_altitude = defaultdict(list)  # NEW: index by altitude
        self.drone_by_loc_alt = defaultdict(list)   # NEW: index by (loc, alt)
            ...
            for alt in self.altitudes:
                ...
                for img in sorted(os.listdir(ad)):
                    if img.endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        self.drone_by_location[li].append(idx)
                        self.drone_by_altitude[alt].append(idx)
                        self.drone_by_loc_alt[(li, alt)].append(idx)
        print(f"  [{mode}] {len(self.samples)} samples, {len(self.locations)} locations")
        for alt in self.altitudes:
            print(f"    Altitude {alt}m: {len(self.drone_by_altitude[alt])} samples")
    ...
    def __getitem__(self, idx):
        ...
        return {'drone': d, 'satellite': s, 'label': li,
                'altitude': int(alt), 'alt_idx': alt_idx}


# =============================================================================
# NEW: Altitude-Stratified PK Sampler
# =============================================================================
class AltitudeStratifiedPKSampler:
    """
    PK Sampler that guarantees balanced altitude representation in each batch.

    Standard PK sampler picks P locations, K samples each — altitude is random.
    This sampler ensures each altitude gets ~P/4 locations, then K samples each.
    This guarantees 150m images always appear in each batch (≥25%).

    For hard altitudes (150m), this provides more consistent gradient signal.
    """
    def __init__(self, ds, p=16, k=4, altitudes=None):
        self.ds = ds; self.p = p; self.k = k
        self.altitudes = altitudes or CFG.ALTITUDES
        self.locs = list(ds.drone_by_location.keys())
        # Build location → set of altitudes with data
        self.loc_alts = defaultdict(set)
        for (loc, alt), indices in ds.drone_by_loc_alt.items():
            if len(indices) > 0:
                self.loc_alts[loc].add(alt)

    def __iter__(self):
        locs = self.locs.copy(); random.shuffle(locs)
        p_per_alt = max(self.p // len(self.altitudes), 1)

        batch = []; alt_idx = 0
        for loc in locs:
            # Cycle through altitudes — try to pick samples from the target altitude
            target_alt = self.altitudes[alt_idx % len(self.altitudes)]

            if target_alt in self.loc_alts[loc]:
                # Get samples from target altitude for this location
                indices = self.ds.drone_by_loc_alt[(loc, target_alt)]
            else:
                # Fallback: use any altitude for this location
                indices = self.ds.drone_by_location[loc]
...
# STUDENT MODEL
# =============================================================================
class SPDGeoAHNModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        print(f"  SPDGeo-AHN student: {total/1e6:.1f}M trainable parameters")
...
# =============================================================================
# NEW: Altitude-Weighted ProxyAnchor Loss
# =============================================================================
class AltitudeWeightedProxyAnchorLoss(nn.Module):
    """
    ProxyAnchor loss with per-altitude adaptive weighting.

    Key idea: Track running average of per-altitude proxy similarity as a
    surrogate for R@1. Altitudes with lower surrogate R@1 get higher loss
    weight: w_alt = C / (running_sim_alt + ε)

    This automatically upweights 150m samples (low similarity → harder) and
    downweights 300m samples (high similarity → already well-learned).
    """
    def __init__(self, num_classes, embed_dim, margin=0.1, alpha=32,
                 num_altitudes=4, momentum=0.9):
        super().__init__()
        ...
        self.num_altitudes = num_altitudes; self.momentum = momentum
        # Running similarity per altitude (higher = easier)
        self.register_buffer('running_sim', torch.ones(num_altitudes) * 0.5)

    def forward(self, embeddings, labels, alt_indices):
        """
        embeddings: [B, D] L2-normalized
        labels:     [B] class indices
        alt_indices: [B] altitude index (0-3)
        """
        P = F.normalize(self.proxies, dim=-1)
        sim = embeddings @ P.T  # [B, C]
        one_hot = F.one_hot(labels, self.num_classes).float()

        # Compute per-sample positive similarity (surrogate for quality)
        with torch.no_grad():
            pos_sim = (sim * one_hot).sum(dim=-1)  # [B]
            # Update running similarity per altitude
            for a in range(self.num_altitudes):
                mask_a = alt_indices == a
                if mask_a.sum() > 0:
                    avg_sim_a = pos_sim[mask_a].mean()
                    self.running_sim[a] = (self.momentum * self.running_sim[a] +
                                           (1 - self.momentum) * avg_sim_a)

            # Compute per-sample weight from altitude difficulty
            # Harder altitudes (lower running_sim) get higher weight
            alt_weights = 1.0 / (self.running_sim + 0.1)  # [num_alt]
            alt_weights = alt_weights / alt_weights.mean()  # Normalize to mean=1
            sample_weights = alt_weights[alt_indices]  # [B]

        # Positive term with altitude weighting
        pos_exp = torch.exp(-self.alpha * (sim * one_hot - self.margin))
        pos_exp = pos_exp * one_hot * sample_weights.unsqueeze(-1)
        ...
        # Negative term (standard)
        neg_mask = 1 - one_hot
        neg_exp = torch.exp(self.alpha * (sim * neg_mask + self.margin))
...
# NEW: Cross-Altitude Hard Pair Loss
# =============================================================================
class CrossAltitudeHardPairLoss(nn.Module):
    """
    Explicitly contrasts drone embeddings across extreme altitudes (150m, 300m)
    with their corresponding satellite embedding as anchor.

    For each location in the batch with both 150m and 300m samples:
      L = (1 - cos(drone_150, sat)) + (1 - cos(drone_300, sat))
         + 0.5 * (1 - cos(drone_150, drone_300))

    This forces the model to learn representations that are consistent
    across the hardest altitude pair, directly attacking the 150m gap.
    """
    def forward(self, drone_emb, sat_emb, labels, altitudes):
        """
        drone_emb: [B, D]
        sat_emb:   [B, D]
        labels:    [B]
        altitudes: [B] actual altitude values (150, 200, 250, 300)
        """
        B = drone_emb.size(0)
        device = drone_emb.device

        # Find all unique locations in batch
        ...
        for loc in unique_locs:
            loc_mask = labels == loc
            loc_alts = altitudes[loc_mask]
            loc_drone = drone_emb[loc_mask]
            loc_sat = sat_emb[loc_mask]

            # Check if this location has both 150m and 300m (or nearest extremes)
            has_150 = (loc_alts == 150)
            has_300 = (loc_alts == 300)

            if has_150.sum() > 0 and has_300.sum() > 0:
                # Average embeddings per altitude for stability
                emb_150 = loc_drone[has_150].mean(0)
                emb_300 = loc_drone[has_300].mean(0)
                emb_sat = loc_sat[0]  # All point to same satellite img

                # Cross-altitude → satellite consistency
                d_150_sat = 1 - F.cosine_similarity(emb_150.unsqueeze(0), emb_sat.unsqueeze(0))
                d_300_sat = 1 - F.cosine_similarity(emb_300.unsqueeze(0), emb_sat.unsqueeze(0))
                # Cross-altitude mutual consistency
                d_150_300 = 1 - F.cosine_similarity(emb_150.unsqueeze(0), emb_300.unsqueeze(0))

                total_loss = total_loss + d_150_sat + d_300_sat + 0.5 * d_150_300
                count += 1
...
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    alt_proxy, ema_dist, cross_alt = new_losses
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            # Altitude-weighted ProxyAnchor
            l_proxy = 0.5 * (alt_proxy(d_out['embedding'], labels, alt_idx) +
                             alt_proxy(s_out['embedding'], labels,
                                       torch.zeros_like(alt_idx)))  # sat: altitude=0 placeholder
            ...
            # Cross-altitude hard pair loss
            l_cross_alt = cross_alt(d_out['embedding'], s_out['embedding'], labels, alts)
            ...
            loss = (CFG.LAMBDA_CE          * l_ce +
                    ...
                    CFG.LAMBDA_CROSS_ALT   * l_cross_alt)
        ...
        loss_sums['cross_alt'] += l_cross_alt.item()

    # Log altitude weights
    with torch.no_grad():
        alt_w = 1.0 / (alt_proxy.running_sim + 0.1)
        alt_w = alt_w / alt_w.mean()

    return total_sum / max(n, 1), {k: v / max(n, 1) for k, v in loss_sums.items()}, alt_w
...
def main():
    ...
    print("=" * 65)
    print("  EXP28: SPDGeo-AHN — Altitude-Hardness-Aware Negative Mining")
    print(f"  Base: SPDGeo-DPE (93.59% R@1)")
    print(f"  Novel: AltStratSampler + AltWeightedProxy + CrossAltHardPair")
    ...
    print(f"  Losses: 6 base + AltProxy + EMA + CrossAlt = 9 total")
    print("=" * 65)
    ...
    # NEW: Altitude-stratified sampler
    train_loader = DataLoader(train_ds,
                              batch_sampler=AltitudeStratifiedPKSampler(train_ds, CFG.P_CLASSES, CFG.K_SAMPLES),
                              num_workers=CFG.NUM_WORKERS, pin_memory=True)

    print('\nBuilding models …')
    model = SPDGeoAHNModel(CFG.NUM_CLASSES).to(DEVICE)
    teacher = None
    try:
    ...
    # New losses
    alt_proxy = AltitudeWeightedProxyAnchorLoss(
        CFG.NUM_CLASSES, CFG.EMBED_DIM, margin=CFG.PROXY_MARGIN, alpha=CFG.PROXY_ALPHA,
        num_altitudes=CFG.NUM_ALTITUDES, momentum=CFG.ALT_WEIGHT_MOMENTUM
    ).to(DEVICE)
    ema_dist  = EMADistillationLoss()
    cross_alt = CrossAltitudeHardPairLoss()
    new_losses = (alt_proxy, ema_dist, cross_alt)
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': alt_proxy.parameters(),  'lr': CFG.LR},
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        avg_loss, ld, alt_w = train_one_epoch(model, teacher, ema, train_loader, base_losses,
                                              new_losses, optimizer, scaler, DEVICE, epoch)

        alt_w_str = " ".join([f"{CFG.ALTITUDES[i]}m:{alt_w[i]:.2f}" for i in range(CFG.NUM_ALTITUDES)])
        print(f"Ep {epoch:3d}/{CFG.NUM_EPOCHS} | Loss {avg_loss:.4f} | "
              ...
              f"UAPA {ld['uapa']:.3f} | "
              f"Proxy {ld['proxy']:.3f}  EMA {ld['ema']:.3f}  XAlt {ld['cross_alt']:.3f} | "
              f"AltW: [{alt_w_str}] | LR {cur_lr:.2e}")
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            # Log per-altitude progress
            if per_alt:
                gap = per_alt.get(300, {}).get('R@1', 0) - per_alt.get(150, {}).get('R@1', 0)
                print(f"  ► 150m-300m gap: {gap:.2f}%")

            if metrics['R@1'] > best_r1:
                best_r1 = metrics['R@1']
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                            'metrics': metrics, 'per_alt': per_alt},
                           os.path.join(CFG.OUTPUT_DIR, 'exp28_ahn_best.pth'))
    ...
    print(f'\n{"="*65}')
    print(f'  EXP28: SPDGeo-AHN COMPLETE — Best R@1: {best_r1:.2f}%')
    print(f'{"="*65}')
    ...
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp28_ahn_results.json'), 'w') as f:
