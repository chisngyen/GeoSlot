"""
EXP14: GeoAltBN — Altitude-Conditioned Batch Normalization
============================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny with Altitude-Conditioned BN
Novel:    1) Replace standard BN with altitude-conditioned BN in bottleneck
          2) Learn altitude-specific affine transforms (γ_alt, β_alt)
          3) Altitude-Aware InfoNCE — scale contrastive temperature by altitude difficulty
Insight:  150m and 300m drone views have VERY different feature distributions.
          Standard BN averages across all altitudes → suboptimal.
Expected: 82-86% R@1 (minimal arch change, strong motivation)
"""
...
class Config:
    ...
    MARGIN=0.3
    ALTITUDES=["150","200","250","300"]; NUM_ALTITUDES=4
...
class SUES200Dataset(Dataset):
    def __init__(self, root, mode="train", altitudes=None, transform=None, train_locs=None, test_locs=None):
        ...
        self.altitude_to_idx = {a:i for i,a in enumerate(Config.ALTITUDES)}
    ...
    def __getitem__(self, idx):
        ...
        alt_idx = self.altitude_to_idx.get(alt, 0)
        return {'drone':d,'satellite':s,'label':li,'altitude':int(alt),'alt_idx':alt_idx}
...
# === NEW: Altitude-Conditioned Batch Normalization ===
class AltitudeConditionedBN(nn.Module):
    """Conditional BN that uses altitude-specific affine parameters.
    Shared running stats but per-altitude γ and β.
    Only ~2K extra params for 4 altitudes."""
    def __init__(self, num_features, num_altitudes=4):
        super().__init__()
        self.bn = nn.BatchNorm1d(num_features, affine=False)
        # Per-altitude affine params
        self.gammas = nn.Parameter(torch.ones(num_altitudes, num_features))
        self.betas = nn.Parameter(torch.zeros(num_altitudes, num_features))
        # Default affine (for satellite/unknown altitude)
        self.default_gamma = nn.Parameter(torch.ones(num_features))
        self.default_beta = nn.Parameter(torch.zeros(num_features))

    def forward(self, x, alt_idx=None):
        x = self.bn(x)
        if alt_idx is not None and self.training:
            # Apply altitude-specific affine
            gamma = self.gammas[alt_idx]  # [B, D]
            beta = self.betas[alt_idx]    # [B, D]
            return gamma * x + beta
        else:
            return self.default_gamma * x + self.default_beta

class MobileGeoStudent(nn.Module):
    def __init__(self, nc, ed=768, dpr=0.1, num_altitudes=4):
        ...
        # Altitude-Conditioned bottleneck
        self.bottleneck_linear = nn.Linear(768, ed)
        self.bottleneck_bn = AltitudeConditionedBN(ed, num_altitudes)
        self.bottleneck_relu = nn.ReLU(True)
        self.classifier = nn.Linear(ed, nc)

    def forward(self, x, return_all=False, alt_idx=None):
        ff, so = self.backbone(x)
        sl = [h(f) for h, f in zip(self.aux_heads, so)]
        emb = self.bottleneck_linear(ff)
        emb = self.bottleneck_bn(emb, alt_idx)
        emb = self.bottleneck_relu(emb)
...
class AltitudeConsistencyLoss(nn.Module):
    """Features from same location but different altitudes should be close.
    Within a batch, find same-location pairs across altitudes and align."""
    def forward(self, feats, labels, altitudes):
        loss = 0.0; count = 0
        unique_labels = labels.unique()
        for l in unique_labels:
            mask = labels == l
            loc_feats = F.normalize(feats[mask], dim=1)
            loc_alts = altitudes[mask]
            if loc_alts.unique().numel() > 1:
                # Mean feature for this location (altitude invariant)
                mean_feat = loc_feats.mean(0, keepdim=True)
                loss += (1 - F.cosine_similarity(loc_feats, mean_feat)).mean()
                count += 1
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.alt_consist=AltitudeConsistencyLoss()
    def forward(self,do,so,l,alt,td=None,ts=None):
        losses={}
        ...
        losses['alt_consist']=self.cfg.LAMBDA_ALT_CONSIST*self.alt_consist(do['embedding_normed'],l,alt)
...
def train_one_epoch(model,teacher,tl,crit,opt,scaler,dev,epoch,cfg=Config):
    ...
    for bi,b in enumerate(tl):
        di=b['drone'].to(dev); si=b['satellite'].to(dev); l=b['label'].to(dev)
        alt=b['altitude'].to(dev); alt_idx=b['alt_idx'].to(dev)
        opt.zero_grad()
        with torch.amp.autocast('cuda',enabled=cfg.USE_AMP):
            do=model(di,return_all=True,alt_idx=alt_idx)
            so=model(si,return_all=True,alt_idx=None)  # satellite: no altitude
            td=ts=None
            if teacher:
                with torch.no_grad(): td=teacher(di); ts=teacher(si)
            loss,ld=crit(do,so,l,alt,td,ts)
...
def main():
    ...
    model=MobileGeoStudent(nc,Config.EMBED_DIM,num_altitudes=Config.NUM_ALTITUDES).to(dev)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp14.pth')); print("  ★ Best!")
