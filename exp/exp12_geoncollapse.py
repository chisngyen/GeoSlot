"""
EXP12: GeoNCollapse — Neural Collapse-Inspired Knowledge Distillation
======================================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) Neural Collapse Structure Transfer — distill ETF simplex geometry from teacher
          2) Prototype Alignment Loss — align student class prototypes to teacher's collapsed structure
          3) Equiangular Tight Frame Regularization — encourage maximal separation in feature space
Ref:      NCKD (AAAI 2025) — adapted to cross-view geo-localization
Expected: 82-86% R@1 (strong theoretical backing, safe addition to baseline)
"""
...
class Config:
    ...
    LAMBDA_NC_PROTO=0.3    # Prototype alignment
    LAMBDA_NC_ETF=0.1      # ETF regularization
...
# === CONVNEXT-TINY + STUDENT (same as baseline) ===
...
def drop_path(x, dp=0., tr=False):
    ...
    kp=1-dp; sh=(x.shape[0],)+(1,)*(x.ndim-1); rt=kp+torch.rand(sh,dtype=x.dtype,device=x.device); rt.floor_()
    return x.div(kp)*rt
...
class ConvNeXtTiny(nn.Module):
    def __init__(self, in_c=3, depths=[3,3,9,3], dims=[96,192,384,768], dpr=0., lsi=1e-6):
        ...
        for i in range(3):
            self.downsample_layers.append(nn.Sequential(LayerNorm(dims[i],df="channels_first"),nn.Conv2d(dims[i],dims[i+1],2,stride=2)))
...
def load_convnext_pretrained(m):
    ...
    except Exception as e: print(f"Pretrained:{e}")
    return m
...
class MobileGeoStudent(nn.Module):
    def __init__(self, nc, ed=768, dpr=0.1):
        super().__init__(); self.backbone=load_convnext_pretrained(ConvNeXtTiny(dpr=dpr))
...
# ============================================================================
# LOSSES (baseline + Neural Collapse novelty)
# ============================================================================
...
class SymmetricInfoNCELoss(nn.Module):
    ...
    def forward(self,df,sf,l):
        df=F.normalize(df,1); sf=F.normalize(sf,1); sim=df@sf.T/self.t
        l=l.view(-1,1); pm=l.eq(l.T).float()
...
# === NEW: Neural Collapse Prototype Alignment ===
class NeuralCollapsePrototypeLoss(nn.Module):
    """Align student prototypes (classifier weights) to teacher's NC structure.
    In NC, class prototypes form an Equiangular Tight Frame (ETF) — maximally separated.
    We transfer this geometric structure from teacher to student.

    Ref: NCKD (AAAI 2025) — adapted for cross-view geo-localization.
    """
    def __init__(self, num_classes, embed_dim=768, momentum=0.99):
        super().__init__()
        self.num_classes = num_classes
        self.momentum = momentum
        # Running mean prototypes for teacher (EMA)
        self.register_buffer('teacher_prototypes', torch.zeros(num_classes, embed_dim))
        self.register_buffer('proto_count', torch.zeros(num_classes))

    @torch.no_grad()
    def update_teacher_prototypes(self, teacher_feats, labels):
        """Update EMA prototypes from teacher features"""
        for c in labels.unique():
            mask = labels == c
            class_feats = teacher_feats[mask].mean(0)
            if self.proto_count[c] == 0:
                self.teacher_prototypes[c] = class_feats
            else:
                self.teacher_prototypes[c] = (self.momentum * self.teacher_prototypes[c] +
                                              (1 - self.momentum) * class_feats)
            self.proto_count[c] += 1

    def forward(self, student_feats, labels, classifier_weight):
        """
        Args:
            student_feats: [B, D] student embeddings
            labels: [B] class labels
            classifier_weight: [C, D] student classifier weights (≈ prototypes)
        """
        # Active prototypes (classes seen in this batch)
        active_classes = labels.unique()
        if len(active_classes) < 2:
            return torch.tensor(0.0, device=student_feats.device)

        # Get teacher prototypes for active classes
        t_protos = F.normalize(self.teacher_prototypes[active_classes], dim=1)  # [K, D]
        s_protos = F.normalize(classifier_weight[active_classes], dim=1)       # [K, D]

        # Pairwise similarity structure alignment
        with torch.no_grad():
            t_sim = t_protos @ t_protos.T  # [K, K]
        s_sim = s_protos @ s_protos.T      # [K, K]

        # Match similarity structures (teacher's NC geometry → student)
        loss_struct = F.mse_loss(s_sim, t_sim)

        # Individual prototype alignment (student features collapse to class means)
        loss_collapse = 0.0
        for c in active_classes:
            mask = labels == c
            class_feats = F.normalize(student_feats[mask], dim=1)
            class_proto = s_protos[active_classes == c]  # [1, D]
            # Intra-class collapse: features → prototype
            loss_collapse += (1 - (class_feats @ class_proto.T).mean())
        loss_collapse /= len(active_classes)

        return loss_struct + 0.5 * loss_collapse


# === NEW: ETF Regularization ===
class ETFRegularization(nn.Module):
    """Equiangular Tight Frame Regularization.
    Encourages classifier weights to form maximally separated directions.
    In ideal ETF, pairwise cosine similarity = -1/(C-1) for C classes."""
    def __init__(self, num_classes):
        super().__init__()
        self.target_sim = -1.0 / (num_classes - 1)  # Ideal ETF angle

    def forward(self, classifier_weight):
        W = F.normalize(classifier_weight, dim=1)  # [C, D]
        sim = W @ W.T  # [C, C]
        # Mask out diagonal
        mask = 1 - torch.eye(sim.size(0), device=sim.device)
        off_diag = sim * mask
        # Push off-diagonal similarities toward target
        target = self.target_sim * mask
        return F.mse_loss(off_diag, target)
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.nc_proto=NeuralCollapsePrototypeLoss(nc, cfg.EMBED_DIM)
        self.etf_reg=ETFRegularization(nc)

    def forward(self,do,so,l,td=None,ts=None,classifier_weight=None):
        losses={}
        ...
        if td is not None:
            ...
            # Update teacher prototypes
            self.nc_proto.update_teacher_prototypes(td, l)
            self.nc_proto.update_teacher_prototypes(ts, l)
            # NC prototype alignment
            if classifier_weight is not None:
                losses['nc_proto']=self.cfg.LAMBDA_NC_PROTO*(
                    self.nc_proto(do['embedding'], l, classifier_weight)+
                    self.nc_proto(so['embedding'], l, classifier_weight))
        # ETF regularization on classifier weights
        if classifier_weight is not None:
            losses['etf_reg']=self.cfg.LAMBDA_NC_ETF*self.etf_reg(classifier_weight)
...
def train_one_epoch(model,teacher,tl,crit,opt,scaler,dev,epoch,cfg=Config):
    ...
    for bi,b in enumerate(tl):
        ...
        with torch.amp.autocast('cuda',enabled=cfg.USE_AMP):
            ...
            cw=model.classifier.weight  # pass classifier weights for NC loss
            loss,ld=crit(do,so,l,td,ts,classifier_weight=cw)
...
def main():
    ...
    print("="*60+"\nEXP12: GeoNCollapse — Neural Collapse-Inspired KD (AAAI'25)\n"+"="*60)
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp12.pth')); print("  ★ Best!")
