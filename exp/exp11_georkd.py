"""
EXP11: GeoRKD — Same DINOv2-B Teacher + Novel Losses Only (RKD + CRD + DarkRank)
==================================================================================
Teacher:  DINOv2 ViT-B/14 (SAME as baseline — 86M, 768-dim)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    Only loss modifications (no architecture/teacher change):
          1) Relational Knowledge Distillation (RKD) — pairwise distance + angle
          2) Contrastive Representation Distillation (CRD) — contrastive alignment
          3) DarkRank — ranking-preserving distillation
          This isolates whether novel LOSSES alone can beat baseline.
Expected: 82-86% R@1 (lowest risk — same teacher, proven pipeline)
"""
...
class Config:
    ...
    LAMBDA_RKD=0.25       # Relational KD
    LAMBDA_CRD=0.2        # Contrastive Representation Distillation
    LAMBDA_DARKRANK=0.15   # DarkRank
...
# ============================================================================
# SAME TEACHER AS BASELINE (DINOv2 ViT-B/14)
# ============================================================================
class DINOv2Teacher(nn.Module):
    def __init__(self, ntb=2):
        super().__init__()
        print("Loading DINOv2-Base teacher (same as baseline)...")
        self.model=torch.hub.load('facebookresearch/dinov2','dinov2_vitb14')
        self.num_channels=768
        for p in self.model.parameters(): p.requires_grad=False
        for blk in self.model.blocks[-ntb:]:
            for p in blk.parameters(): p.requires_grad=True
        print("  DINOv2-Base loaded (768-dim)")
    @torch.no_grad()
    def forward(self,x):
...
# ============================================================================
# LOSSES (baseline losses + 3 novel losses)
# ============================================================================
...
# === NEW: Relational Knowledge Distillation ===
class RKDLoss(nn.Module):
    """Transfer pairwise distance + angle relationships (Park et al., CVPR 2019)"""
    def __init__(self, w_dist=1.0, w_angle=2.0):
        super().__init__(); self.wd=w_dist; self.wa=w_angle
    def _pdist(self, e):
        sq=e.pow(2).sum(1); return (sq.unsqueeze(1)+sq.unsqueeze(0)-2*e@e.t()).clamp(min=1e-12).sqrt()
    def forward(self, student, teacher):
        with torch.no_grad():
            td=self._pdist(teacher); mu=td[td>0].mean(); td=td/(mu+1e-8)
        sd=self._pdist(student); mu=sd[sd>0].mean(); sd=sd/(mu+1e-8)
        loss_d=F.smooth_l1_loss(sd,td)
        with torch.no_grad():
            diff_t=teacher.unsqueeze(0)-teacher.unsqueeze(1)
            t_angle=torch.bmm(F.normalize(diff_t,2,2),F.normalize(diff_t,2,2).transpose(1,2)).view(-1)
        diff_s=student.unsqueeze(0)-student.unsqueeze(1)
        s_angle=torch.bmm(F.normalize(diff_s,2,2),F.normalize(diff_s,2,2).transpose(1,2)).view(-1)
        loss_a=F.smooth_l1_loss(s_angle,t_angle)
        return self.wd*loss_d+self.wa*loss_a

# === NEW: Contrastive Representation Distillation ===
class CRDLoss(nn.Module):
    """Project student & teacher to shared space, align via InfoNCE (Tian et al., ICLR 2020)"""
    def __init__(self, sd=768, td=768, proj_dim=256, temperature=0.07):
        super().__init__()
        self.s_proj=nn.Sequential(nn.Linear(sd,proj_dim),nn.ReLU(),nn.Linear(proj_dim,proj_dim))
        self.t_proj=nn.Sequential(nn.Linear(td,proj_dim),nn.ReLU(),nn.Linear(proj_dim,proj_dim))
        self.t=temperature
    def forward(self, student_feat, teacher_feat):
        s=F.normalize(self.s_proj(student_feat),1)
        t=F.normalize(self.t_proj(teacher_feat.detach()),1)
        B=s.size(0); sim=s@t.T/self.t
        labels=torch.arange(B,device=s.device)
        return 0.5*(F.cross_entropy(sim,labels)+F.cross_entropy(sim.T,labels))

# === NEW: DarkRank Loss ===
class DarkRankLoss(nn.Module):
    """Transfer ranking knowledge: match softmax-of-similarities (Chen et al., AAAI 2018)"""
    def __init__(self, alpha=3.0, beta=3.0):
        super().__init__(); self.a=alpha; self.b=beta
    def forward(self, student, teacher):
        s_sim = -(torch.cdist(student, student, p=2))
        with torch.no_grad():
            t_sim = -(torch.cdist(teacher, teacher, p=2))
        s_prob=F.log_softmax(s_sim*self.a, dim=1)
        with torch.no_grad():
            t_prob=F.softmax(t_sim*self.b, dim=1)
...
class MobileGeoLoss(nn.Module):
    def __init__(self, nc, cfg=Config):
        ...
        self.rkd=RKDLoss()       # NEW
        self.crd=CRDLoss()       # NEW
        self.darkrank=DarkRankLoss()  # NEW
    ...
    def forward(self,do,so,l,td=None,ts=None):
        ...
        if td is not None:
            ...
            losses['rkd']=self.cfg.LAMBDA_RKD*(
                self.rkd(do['embedding_normed'],F.normalize(td,1))+self.rkd(so['embedding_normed'],F.normalize(ts,1)))
            losses['crd']=self.cfg.LAMBDA_CRD*(
                self.crd(do['final_feature'],td)+self.crd(so['final_feature'],ts))
            losses['darkrank']=self.cfg.LAMBDA_DARKRANK*(
                self.darkrank(do['embedding_normed'],F.normalize(td,1))+self.darkrank(so['embedding_normed'],F.normalize(ts,1)))
...
def main():
    ...
    print("="*60+"\nEXP11: GeoRKD — Same DINOv2-B + Novel Losses (RKD+CRD+DarkRank)\n"+"="*60)
    ...
    except Exception as e: print(f"DINOv2 failed:{e}"); teacher=None
        ...
        if (ep+1)%5==0 or ep==Config.NUM_EPOCHS-1:
            ...
            if r['R@1']>best:
                best=r['R@1']; torch.save({'epoch':ep,'model':model.state_dict(),'r1':r['R@1'],'ap':ap},
                    os.path.join(Config.OUTPUT_DIR,'best_model_exp11.pth')); print("  ★ Best!")
