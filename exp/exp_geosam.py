#!/usr/bin/env python3
"""
GeoSAM: Sharpness-Aware Minimization for Cross-View Drone Geo-Localization
============================================================================
Built on: baseline.py (MobileGeo — 82.35% R@1)
Novel contribution: SAM optimizer + EMA + Gradient Centralization
Architecture: IDENTICAL to baseline. Novelty = optimizer only.
NOTE: SAM uses two forward passes — GradScaler is INCOMPATIBLE. Use bfloat16.
"""
...
class SUES200Dataset(Dataset):
    def __init__(self,root,mode="train",altitudes=None,transform=None,train_locs=None,test_locs=None):
        ...
        self.locations=[f"{l:04d}" for l in locs]
...
# === STUDENT (identical to baseline) ===
class GeoSAMStudent(nn.Module):
    ...
    def forward(self,x,return_all=False):
        ...
        emb=self.bottleneck(f); en=F.normalize(emb,p=2,dim=1); lg=self.classifier(emb)
        if return_all: return {'embedding':emb,'embedding_normed':en,'logits':lg,'stage_logits':slg,'stage_features':sts,'final_feature':f}
        return en, lg
    ...
    def first_step(self,zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                self.state[p]["old_p"]=p.data.clone(); p.add_(p.grad*scale.to(p))
...
class GeoSAMLoss(nn.Module):
    ...
    def forward(self,do,so,labels,td=None,ts=None):
        ...
        ce+=self.ce(so['logits'],labels); L['ce']=ce
...
# === TRAINING ===
def train_one_epoch(model,teacher,loader,criterion,optimizer,ema,device):
    model.train()
    ...
    for bi,batch in enumerate(loader):
        ...
        def compute_loss():
            with autocast("cuda",dtype=torch.bfloat16):
                ...
                return criterion(do,so,labels,td,ts)
        loss1,L=compute_loss(); loss1.backward(); centralize_gradients(optimizer)
        torch.nn.utils.clip_grad_norm_(model.parameters(),5.0); optimizer.first_step(zero_grad=True)
        loss2,_=compute_loss(); loss2.backward(); centralize_gradients(optimizer)
        ...
        if bi%10==0: print(f"  B{bi}/{len(loader)} L={loss1.item():.4f}")
...
def main():
    ...
    print("="*60); print("GeoSAM: SAM Optimizer + EMA — SUES-200"); print("="*60)
    ...
    model=GeoSAMStudent(num_classes=len(Config.TRAIN_LOCS)).to(device)
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        avg_l,ld=train_one_epoch(model,teacher,loader,criterion,optimizer,ema,device)
        ...
        if (epoch+1)%5==0 or epoch==Config.NUM_EPOCHS-1:
            ...
            if rec['R@1']>best_r1:
                best_r1=rec['R@1']
                torch.save({'epoch':epoch,'model':model.state_dict(),'r1':best_r1},os.path.join(Config.OUTPUT_DIR,'geosam_best.pth'))
