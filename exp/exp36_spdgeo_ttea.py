...
# =============================================================================
# Base:    SPDGeo-DPEA (93.80% R@1) — loads checkpoint
# Novel:   INFERENCE-ONLY improvements with ALL EXP30 bugs fixed:
#          1) Multi-Crop Ensemble at 3 resolutions [280, 336, 392]
#             ALL are multiples of 14 (DINOv2 patch_size) — fixes EXP30 bug
#          2) Batch-Level Tent Adaptation — entropy minimization on part
#             prototypes over FULL test set, not reset per-batch (EXP30 bug)
#          3) Altitude-Aware Inference — passes alt_idx during multi-crop
#             extraction (DPEA uses DeepFiLM, needs altitude at inference)
#          4) EMA Model Ensemble — if EMA checkpoint available
#
# Fixes over EXP30:
#   BUG 1: Crop sizes [288,336,384] → [280,336,392] (all 14×N)
#   BUG 2: Prototype reset per-batch killed adaptation → cumulative adaptation
#   BUG 3: Per-image Tent unstable → batch-level over full test dataset
#   BUG 4: No altitude conditioning during inference → pass alt_idx
#
# This experiment LOADS a trained DPEA model and evaluates with TTE.
# It does NOT train from scratch — it's a pure inference experiment.
# If no checkpoint found, falls back to training DPEA first.
# =============================================================================
...
class Config:
    ...
    # TTE hyperparameters — ALL FIXED from EXP30
    MULTI_CROP_SIZES   = [280, 336, 392]   # ALL multiples of 14 ✓
    CROP_WEIGHTS       = [0.25, 0.50, 0.25]
    USE_EMA_ENSEMBLE   = True
    EMA_ENSEMBLE_ALPHA = 0.5

    # Tent: batch-level adaptation (FIXED from EXP30)
    TENT_STEPS         = 5     # More steps since batch-level is more stable
    TENT_LR            = 5e-5  # Lower LR for stability
    TENT_ENABLED       = True
    TENT_BATCH_SIZE    = 64    # Process test data in batches for Tent

    # Checkpoint — tries DPEA first, falls back to DPE
    CHECKPOINT_PATH    = "/kaggle/working/exp26_dpea_best.pth"
    DPE_CHECKPOINT     = "/kaggle/working/exp20_dpe_best.pth"
...
# =============================================================================
# MODEL — DPEA architecture (needed to load checkpoint)
# =============================================================================
...
class SPDGeoDPEAModel(nn.Module):
    ...
    def extract_with_parts(self, x, alt_idx=None):
...
# =============================================================================
# FIXED: Batch-Level Tent Adaptation
# =============================================================================
class BatchTentAdaptation:
    """
    Batch-level Tent adaptation (Wang et al., ICLR 2021).

    KEY FIX from EXP30: Instead of resetting prototypes per-batch, we
    accumulate adaptation across ALL test batches. Prototypes gradually
    sharpen over the full test set.

    Protocol:
    1. Before evaluation: run Tent adaptation on ALL test images
    2. This updates prototypes to minimize assignment entropy
    3. Then run evaluation with adapted prototypes
    4. Reset prototypes after evaluation
    """
    def __init__(self, model, lr=5e-5, steps=5):
        self.model = model
        self.lr = lr
        self.steps = steps
        self.orig_prototypes = model.part_disc.prototypes.data.clone()

    def adapt_on_dataset(self, dataloader, device):
        """
        Run Tent adaptation steps over the full test dataset.
        Adapts prototypes cumulatively (no reset between batches).
        """
        self.model.eval()
        ...
        for step in range(self.steps):
            ...
            for batch in dataloader:
                ...
                # Forward through part discovery only
                patches, cls_tok, hw = self.model.backbone(imgs)
                parts = self.model.part_disc(patches, hw, alt_idx=alt_idx)
                assignment = parts['assignment']

                # Batch-level entropy: average assignment across batch, then entropy
                assign_probs = assignment.mean(dim=1)  # [B, K]
                batch_avg = assign_probs.mean(dim=0)    # [K] — global part usage
                entropy = -(batch_avg * (batch_avg + 1e-8).log()).sum()
                entropy.backward()
            ...
            print(f"    Tent step {step+1}/{self.steps}: avg entropy = {avg_entropy:.4f}")

        self.model.part_disc.prototypes.requires_grad_(False)
        print(f"    Tent adaptation complete.")

    def reset(self):
        """Reset prototypes to original checkpoint state."""
        self.model.part_disc.prototypes.data.copy_(self.orig_prototypes)


# =============================================================================
# Multi-Crop Feature Extraction (altitude-aware)
# =============================================================================
@torch.no_grad()
def multi_crop_extract_batch(model, images, alt_idx, crop_sizes, crop_weights, device):
    """
    Extract features at multiple resolutions for a batch of images.
    This is more efficient than per-image extraction.

    images: [B, C, H_orig, W_orig] tensor (original resolution)
    alt_idx: [B] or None
    Returns: [B, D] L2-normalized embeddings
    """
    all_feats = []
    for sz, w in zip(crop_sizes, crop_weights):
        # Resize batch to target resolution
        resized = F.interpolate(images, size=(sz, sz), mode='bilinear', align_corners=False)
        feat = model.extract_embedding(resized, alt_idx=alt_idx)
...
@torch.no_grad()
def multi_crop_extract_pil(model, image_pil, alt_idx_val, crop_sizes, crop_weights, device):
    """
    Extract features at multiple resolutions from a single PIL image.
    alt_idx_val: int (e.g., 0 for 150m) or None for satellite
    """
    all_feats = []
    for sz, w in zip(crop_sizes, crop_weights):
        tf = get_transforms("test", img_size=sz)
        img_t = tf(image_pil).unsqueeze(0).to(device)
        ai = torch.tensor([alt_idx_val], device=device) if alt_idx_val is not None else None
        feat = model.extract_embedding(img_t, alt_idx=ai)
        all_feats.append(feat * w)
...
def evaluate_baseline(model, test_ds, device):
    """Standard single-scale evaluation (same as DPEA)."""
    model.eval()
...
@torch.no_grad()
def evaluate_multicrop(model, test_ds, device):
    """Multi-crop evaluation with altitude-aware extraction."""
    model.eval()

    # Drone: multi-crop with altitude
    print("  Extracting drone features (multi-crop + altitude) …")
    drone_feats, drone_labels, drone_alts = [], [], []
    for dp, sp, li, alt in tqdm(test_ds.samples, desc="Drone multi-crop"):
        try: d_pil = Image.open(dp).convert('RGB')
        except Exception: d_pil = Image.new('RGB', (CFG.IMG_SIZE, CFG.IMG_SIZE), (128,128,128))
        alt_val = CFG.ALT_TO_IDX.get(alt, 0)
        feat = multi_crop_extract_pil(model, d_pil, alt_val,
                                       CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, device)
    ...
    # Satellite: multi-crop WITHOUT altitude (alt_idx=None → mean FiLM)
    ...
    for loc in tqdm(all_locs, desc="Satellite multi-crop"):
        ...
        except Exception: s_pil = Image.new('RGB', (CFG.IMG_SIZE, CFG.IMG_SIZE), (128,128,128))
        feat = multi_crop_extract_pil(model, s_pil, None,
                                       CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, device)
        sat_feats.append(feat.cpu())
        if loc in test_ds.location_to_idx: sat_labels.append(test_ds.location_to_idx[loc])
...
# =============================================================================
# FALLBACK TRAINING (if no checkpoint available)
# =============================================================================
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    proxy_anchor, ema_dist, alt_consist = new_losses
    ...
    for batch in tqdm(loader, desc=f'Ep{epoch:3d}', leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=CFG.USE_AMP):
            ...
            l_alt = alt_consist(d_out['embedding'], labels, alts)
            loss = (CFG.LAMBDA_CE * l_ce + CFG.LAMBDA_INFONCE * l_nce + CFG.LAMBDA_CONSISTENCY * l_con +
                    ...
                    CFG.LAMBDA_PROXY * l_proxy + CFG.LAMBDA_EMA_DIST * l_ema + CFG.LAMBDA_ALT_CONSIST * l_alt)
...
def main():
    ...
    print("=" * 70)
    print("  EXP36: SPDGeo-TTEA — Test-Time Ensemble for DPEA (Fixed)")
    print(f"  Fixes: crop sizes=[280,336,392] (14×N), batch-level Tent, altitude-aware")
    print(f"  Components: MultiCrop + BatchTent(steps={CFG.TENT_STEPS}, lr={CFG.TENT_LR})")
    ...
    # Build model
    print('\nBuilding model …')
    model = SPDGeoDPEAModel(CFG.NUM_CLASSES).to(DEVICE)

    # Try to load DPEA checkpoint, then DPE, then train from scratch
    checkpoint_loaded = False
    for ckpt_path in [CFG.CHECKPOINT_PATH, CFG.DPE_CHECKPOINT]:
        if os.path.exists(ckpt_path):
            ...
            try:
                ...
                print(f"  [WARN] State dict mismatch: {e}")
                print(f"  Trying next checkpoint …")

    if not checkpoint_loaded:
        print("\n  [FALLBACK] No checkpoint found — training DPEA from scratch …")
        ...
        new_losses = (proxy_anchor, ema_dist, alt_consist)
            ...
            if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
                ...
                if metrics['R@1'] > best_r1:
                    ...
                    torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                               ...
                               os.path.join(CFG.OUTPUT_DIR, 'exp36_ttea_trained.pth'))

        ckpt = torch.load(os.path.join(CFG.OUTPUT_DIR, 'exp36_ttea_trained.pth'), map_location=DEVICE, weights_only=False)
        model.load_state_dict(ckpt['model_state_dict'])
        print(f"\n  Trained DPEA best R@1: {best_r1:.2f}%")

    model.eval()
    results = {}

    # ======== 1. Baseline (single-scale, no tricks) ========
    print("\n[1/4] Baseline evaluation (single-scale) …")
    base_overall, base_per_alt = evaluate_baseline(model, test_ds, DEVICE)
    print_results("BASELINE (Single-Scale + Altitude)", base_overall, base_per_alt)
    results['baseline'] = base_overall

    # ======== 2. Multi-Crop Only ========
    print("[2/4] Multi-crop evaluation (3 scales, altitude-aware) …")
    mc_overall, mc_per_alt = evaluate_multicrop(model, test_ds, DEVICE)
    print_results("MULTI-CROP [280,336,392] + ALTITUDE", mc_overall, mc_per_alt)
    results['multi_crop'] = mc_overall

    # ======== 3. Multi-Crop + Tent ========
    if CFG.TENT_ENABLED:
        print(f"[3/4] Multi-crop + Batch Tent ({CFG.TENT_STEPS} steps) …")
        # Create Tent adapter and run adaptation
        tent = BatchTentAdaptation(model, lr=CFG.TENT_LR, steps=CFG.TENT_STEPS)
        tent_loader = DataLoader(test_ds, batch_size=CFG.TENT_BATCH_SIZE, shuffle=False,
                                 num_workers=CFG.NUM_WORKERS, pin_memory=True)
        print("  Running batch-level Tent adaptation on test set …")
        tent.adapt_on_dataset(tent_loader, DEVICE)

        # Evaluate with adapted prototypes
        tent_mc_overall, tent_mc_per_alt = evaluate_multicrop(model, test_ds, DEVICE)
        print_results("MULTI-CROP + BATCH TENT", tent_mc_overall, tent_mc_per_alt)
        results['multicrop_tent'] = tent_mc_overall

        # Reset for next evaluation
        tent.reset()

    # ======== 4. Full TTE: Multi-Crop + Tent + EMA ========
    if CFG.USE_EMA_ENSEMBLE:
        print("[4/4] Full TTE (multi-crop + Tent + EMA ensemble) …")
        # Re-run Tent
        tent2 = BatchTentAdaptation(model, lr=CFG.TENT_LR, steps=CFG.TENT_STEPS)
        tent2.adapt_on_dataset(tent_loader, DEVICE)

        # Create EMA model (deep copy of adapted model)
        ema_model = copy.deepcopy(model)
        ema_model.eval()

        # Extract features with model + EMA ensemble
        print("  Extracting drone features (multi-crop + EMA ensemble) …")
        drone_feats, drone_labels, drone_alts = [], [], []
        for dp, sp, li, alt in tqdm(test_ds.samples, desc="Drone TTE"):
            try: d_pil = Image.open(dp).convert('RGB')
            except Exception: d_pil = Image.new('RGB', (CFG.IMG_SIZE, CFG.IMG_SIZE), (128,128,128))
            alt_val = CFG.ALT_TO_IDX.get(alt, 0)

            feat1 = multi_crop_extract_pil(model, d_pil, alt_val,
                                           CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, DEVICE)
            feat2 = multi_crop_extract_pil(ema_model, d_pil, alt_val,
                                           CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, DEVICE)
            alpha = CFG.EMA_ENSEMBLE_ALPHA
            feat = F.normalize((1 - alpha) * feat1 + alpha * feat2, dim=-1)
            drone_feats.append(feat.cpu()); drone_labels.append(li); drone_alts.append(int(alt))

        print("  Extracting satellite features (multi-crop + EMA ensemble) …")
        all_locs = [f"{l:04d}" for l in range(1, 201)]
        sat_feats, sat_labels = [], []; distractor_cnt = 0
        for loc in tqdm(all_locs, desc="Satellite TTE"):
            sp_path = os.path.join(test_ds.sat_dir, loc, "0.png")
            if not os.path.exists(sp_path): continue
            try: s_pil = Image.open(sp_path).convert('RGB')
            except Exception: s_pil = Image.new('RGB', (CFG.IMG_SIZE, CFG.IMG_SIZE), (128,128,128))
            feat1 = multi_crop_extract_pil(model, s_pil, None,
                                           CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, DEVICE)
            feat2 = multi_crop_extract_pil(ema_model, s_pil, None,
                                           CFG.MULTI_CROP_SIZES, CFG.CROP_WEIGHTS, DEVICE)
            alpha = CFG.EMA_ENSEMBLE_ALPHA
            feat = F.normalize((1 - alpha) * feat1 + alpha * feat2, dim=-1)
            sat_feats.append(feat.cpu())
        ...
        drone_feats = torch.cat(drone_feats); drone_labels = torch.tensor(drone_labels)
        drone_alts = torch.tensor(drone_alts)
        sat_feats = torch.cat(sat_feats); sat_labels = torch.tensor(sat_labels)

        tte_overall, tte_per_alt = _compute_metrics(drone_feats, drone_labels, drone_alts, sat_feats, sat_labels)
        print_results("FULL TTE (MultiCrop + BatchTent + EMA)", tte_overall, tte_per_alt)
    ...
    # ======== Summary ========
    print(f"\n{'='*75}")
    print(f"  EXP36: TTEA ABLATION SUMMARY")
    print(f"{'='*75}")
    print(f"  {'Method':<40s}  {'R@1':>7s}  {'R@5':>7s}  {'R@10':>7s}  {'mAP':>7s}")
    print(f"  {'-'*65}")
    for name, m in results.items():
        label = name.replace('_', ' ').title()
        print(f"  {label:<40s}  {m['R@1']:6.2f}%  {m['R@5']:6.2f}%  {m['R@10']:6.2f}%  {m['mAP']:6.2f}%")
    print(f"{'='*75}")

    best_method = max(results.items(), key=lambda x: x[1]['R@1'])
    print(f"\n  Best: {best_method[0]} → R@1: {best_method[1]['R@1']:.2f}%")

    with open(os.path.join(CFG.OUTPUT_DIR, 'exp36_ttea_results.json'), 'w') as f:
