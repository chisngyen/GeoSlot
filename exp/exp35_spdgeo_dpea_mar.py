# =============================================================================
# EXP35: SPDGeo-DPEA-MAR - Full Merge (FiLM + MAR + AltPred + Diversity)
# =============================================================================
# Merge:   EXP27 (DeepAltitudeFiLM + AltConsistency + TTE)
#        + EXP34 (MaskedPartReconstruction + AltPred + Diversity)
#
# Architecture: SPDGeo-DPE + DeepAltitudeFiLM + MAR + AltPred + DiversityReg
# Train: 120 locations (standard), Eval: 200-loc full gallery
# EMA decay: 0.996 (faster convergence)
# Total losses: 12
#   (6 base + Proxy + EMA + AltConsist + MaskRecon + AltPred + Diversity)
# =============================================================================
...
def evaluate(model, test_ds, device):
    ...
    for alt in altitudes:
        ...
        if mask.sum() == 0:
            continue
        af = drone_feats[mask]
        al = drone_labels[mask]
        s = af @ sat_feats.T
        _, rk = s.sort(1, descending=True)
        k = af.size(0)
        a1 = a5 = a10 = aap = 0
        for i in range(k):
...
class TentAdaptation:
    ...
    def adapt_and_extract(self, images, alt_idx=None):
        ...
        # Enable grad explicitly for Tent adaptation
...
def multi_crop_extract(model, image_pil, crop_sizes, crop_weights, device, alt_idx=None, tent=None):
    all_feats = []
...
def evaluate_tte(model, ema_model, test_ds, device, tent=None):
    ...
    for sample in tqdm(test_ds.samples, desc="TTE Drone"):
        ...
        try:
            ...
            d_pil = Image.new("RGB", (336, 336), (128, 128, 128))

        alt_idx_val = CFG.ALT_TO_IDX.get(alt, 0)
        alt_t = torch.tensor([alt_idx_val], device=device)

        feat = multi_crop_extract(model, d_pil, crop_sizes, crop_weights, device, alt_idx=alt_t, tent=tent)
        if ema_model is not None:
            ema_feat = multi_crop_extract(ema_model, d_pil, crop_sizes, crop_weights, device, alt_idx=alt_t, tent=None)
    ...
    for loc in tqdm(all_locs, desc="TTE Satellite"):
        ...
        try:
            ...
            s_pil = Image.new("RGB", (336, 336), (128, 128, 128))

        feat = multi_crop_extract(model, s_pil, crop_sizes, crop_weights, device, alt_idx=None, tent=None)
        if ema_model is not None:
            ema_feat = multi_crop_extract(ema_model, s_pil, crop_sizes, crop_weights, device, alt_idx=None, tent=None)
            feat = F.normalize(0.5 * feat + 0.5 * ema_feat, dim=-1)
...
def main():
    ...
    print("  EXP27 (FiLM+AltConsist+TTE) + EXP34 (MAR+AltPred+Diversity)")
    ...
    print(f"  Losses: 12 total | Recon warmup: {CFG.RECON_WARMUP} | EMA: {CFG.EMA_DECAY}")
    print("=" * 65)
    ...
    ckpt_path = os.path.join(CFG.OUTPUT_DIR, "exp35_dpea_mar_best.pth")
    ...
    print(f"\n{'='*65}")
    print(f"  EXP35 COMPLETE - Best R@1: {best_r1:.2f}%")
    print(f"{'='*65}")

    with open(os.path.join(CFG.OUTPUT_DIR, "exp35_dpea_mar_results.json"), "w") as f:
        json.dump(
            {
    ...
    # Optional TTE evaluation
    print("\n" + "=" * 65)
    print("  TTE: Test-Time Ensemble Evaluation")
    print("=" * 65)
    ...
    try:
        ...
        print(f"  [WARN] Full TTE failed: {e}")
    ...
    tte_payload = {
        ...
        "mc_ema": tte_mc_ema,
        "full_tte": tte_full,
        "final_best_r1": max(best_r1, tte_mc["R@1"], tte_mc_ema["R@1"], tte_full["R@1"] if tte_full else -1),
    }

    with open(os.path.join(CFG.OUTPUT_DIR, "exp35_dpea_mar_tte_results.json"), "w") as f:
