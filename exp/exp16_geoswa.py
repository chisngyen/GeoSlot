"""
EXP16: GeoSWA — Stochastic Weight Averaging + Test-Time Augmentation
======================================================================
Teacher:  DINOv2 ViT-B/14 (same as baseline)
Student:  ConvNeXt-Tiny (same as baseline)
Novel:    1) SWA — average weights from last N epochs for flatter minima
          2) TTA at inference — multi-aug query features + confidence aggregation
          3) Cyclical LR for SWA (explore multiple optima)
          No new loss — purely optimization-level improvement.
Expected: 83-87% R@1 (FREE perf boost, mathematically sound)
"""
...
import os, math, random, copy, argparse, numpy as np
...
class Config:
    ...
    LAMBDA_CROSS_DIST=0.3; LAMBDA_ALIGN=0.2
    MARGIN=0.3; ALTITUDES=["150","200","250","300"]
    TRAIN_LOCS=list(range(1,121)); TEST_LOCS=list(range(121,201))
    USE_AMP=True; SEED=42
    # SWA config
    SWA_START=80           # Start SWA at epoch 80
    SWA_LR=0.0005          # SWA learning rate
    SWA_ANNEAL_EPOCHS=5    # Anneal period
    # TTA config
    TTA_AUGMENTS=5          # Number of augmented copies at test time
...
def get_tta_transforms():
    """Multiple augmentation transforms for TTA"""
    norm = T.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])
    return [
        T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),T.ToTensor(),norm]),  # Original
        T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),T.RandomHorizontalFlip(1.0),T.ToTensor(),norm]),  # HFlip
        T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),T.RandomVerticalFlip(1.0),T.ToTensor(),norm]),  # VFlip
        T.Compose([T.Resize((int(Config.IMG_SIZE*1.1),int(Config.IMG_SIZE*1.1))),T.CenterCrop(Config.IMG_SIZE),T.ToTensor(),norm]),  # Zoom
        T.Compose([T.Resize((Config.IMG_SIZE,Config.IMG_SIZE)),T.RandomRotation((90,90)),T.ToTensor(),norm]),  # 90° rot
    ]
...
def drop_path(x, dp=0., tr=False):
    ...
    kp=1-dp; sh=(x.shape[0],)+(1,)*(x.ndim-1); rt=kp+torch.rand(sh,dtype=x.dtype,device=x.device); rt.floor_(); return x.div(kp)*rt
...
class ConvNeXtBlock(nn.Module):
    def __init__(self, dim, dpr=0., lsi=1e-6):
        super().__init__(); self.dwconv=nn.Conv2d(dim,dim,7,padding=3,groups=dim); self.norm=LayerNorm(dim)
        self.pwconv1=nn.Linear(dim,4*dim); self.act=nn.GELU(); self.pwconv2=nn.Linear(4*dim,dim)
        self.gamma=nn.Parameter(lsi*torch.ones(dim)) if lsi>0 else None; self.drop_path=DropPath(dpr) if dpr>0 else nn.Identity()
    def forward(self, x):
        sc=x; x=self.dwconv(x); x=x.permute(0,2,3,1); x=self.norm(x); x=self.pwconv1(x); x=self.act(x); x=self.pwconv2(x)
...
class ConvNeXtTiny(nn.Module):
    def __init__(self, in_c=3, depths=[3,3,9,3], dims=[96,192,384,768], dpr=0., lsi=1e-6):
        ...
        self.downsample_layers.append(nn.Sequential(nn.Conv2d(in_c,dims[0],4,stride=4),LayerNorm(dims[0],df="channels_first")))
        for i in range(3): self.downsample_layers.append(nn.Sequential(LayerNorm(dims[i],df="channels_first"),nn.Conv2d(dims[i],dims[i+1],2,stride=2)))
        dp=[x.item() for x in torch.linspace(0,dpr,sum(depths))]; cur=0; self.stages=nn.ModuleList()
        for i in range(4): self.stages.append(nn.Sequential(*[ConvNeXtBlock(dims[i],dp[cur+j],lsi) for j in range(depths[i])])); cur+=depths[i]
...
class MobileGeoStudent(nn.Module):
    def __init__(self, nc, ed=768, dpr=0.1):
        super().__init__(); self.backbone=load_convnext_pretrained(ConvNeXtTiny(dpr=dpr)); self.dims=[96,192,384,768]
        self.aux_heads=nn.ModuleList([ClassificationHead(d,nc) for d in self.dims])
        self.bottleneck=nn.Sequential(nn.Linear(768,ed),nn.BatchNorm1d(ed),nn.ReLU(True)); self.classifier=nn.Linear(ed,nc)
    def forward(self, x, return_all=False):
        ff,so=self.backbone(x); sl=[h(f) for h,f in zip(self.aux_heads,so)]
        emb=self.bottleneck(ff); en=F.normalize(emb,p=2,dim=1); logits=self.classifier(emb)
        if return_all: return {'embedding':emb,'embedding_normed':en,'logits':logits,'stage_logits':sl,'stage_features':so,'final_feature':ff}
        return en, logits
class DINOv2Teacher(nn.Module):
    def __init__(self, ntb=2):
        super().__init__(); self.model=torch.hub.load('facebookresearch/dinov2','dinov2_vitb14'); self.num_channels=768
...
# === LOSSES (same as baseline — no new losses, novelty is in optimization) ===
...
class SymmetricInfoNCELoss(nn.Module):
    ...
    def forward(self,df,sf,l):
        df=F.normalize(df,1); sf=F.normalize(sf,1); sim=df@sf.T/self.t; l=l.view(-1,1); pm=l.eq(l.T).float()
        return 0.5*(-torch.log((F.softmax(sim,1)*pm).sum(1)/pm.sum(1).clamp(min=1)).mean()
                     -torch.log((F.softmax(sim.T,1)*pm).sum(1)/pm.sum(1).clamp(min=1)).mean())
...
class CrossDistillationLoss(nn.Module):
    ...
    def forward(self,sf,tf):
        s=F.normalize(sf,1); t=F.normalize(tf,1)
...
# ============================================================================
class SWAModel:
    """Manual SWA implementation — averages model weights."""
    ...
    def update(self):
        """Add current model state to running average"""
        if self.swa_state is None:
            self.swa_state = {k: v.clone() for k, v in self.model.state_dict().items()}
        else:
            for k, v in self.model.state_dict().items():
                self.swa_state[k] = (self.swa_state[k] * self.n_averaged + v) / (self.n_averaged + 1)
        self.n_averaged += 1
        print(f"  SWA update #{self.n_averaged}")

    def apply(self):
        """Load averaged weights into model"""
        if self.swa_state is not None:
            self.model.load_state_dict(self.swa_state)
            print(f"  Applied SWA weights (averaged over {self.n_averaged} checkpoints)")

    def update_bn(self, loader, device):
        """Re-compute BN stats after weight averaging"""
        self.apply()
        self.model.train()
        with torch.no_grad():
            for b in loader:
                di = b['drone'].to(device)
                si = b['satellite'].to(device)
                self.model(di)
                self.model(si)
        print("  Updated BN statistics for SWA model")


# ============================================================================
# TTA (Test-Time Augmentation) Evaluation
# ============================================================================
def evaluate_with_tta(model, tds, dev, n_augs=5):
    """Evaluate with Test-Time Augmentation: average features from multiple augmentations."""
    model.eval()
    tta_transforms = get_tta_transforms()[:n_augs]
    sd_path = os.path.join(tds.root, Config.SATELLITE_DIR)

    # Drone features with TTA
    all_drone_feats, all_labels = [], []
    for sample_idx in range(len(tds)):
        dp, sp, li, alt = tds.samples[sample_idx]
        drone_img = Image.open(dp).convert('RGB')

        # Apply each TTA transform and average features
        feats = []
        for t in tta_transforms:
            with torch.no_grad():
                img_t = t(drone_img).unsqueeze(0).to(dev)
                f, _ = model(img_t)
                feats.append(f.cpu())

        # L2-normalize each, then average, then re-normalize
        avg_feat = F.normalize(torch.stack(feats).mean(0), dim=1)
        all_drone_feats.append(avg_feat)
        all_labels.append(li)

        if (sample_idx + 1) % 200 == 0:
            print(f"  TTA progress: {sample_idx+1}/{len(tds)}")

    all_drone_feats = torch.cat(all_drone_feats)
    all_labels = torch.tensor(all_labels)

    # Satellite features (no TTA, just clean)
    tr = get_transforms("test")
    sf, sl, gn = [], [], []
    for loc in [f"{l:04d}" for l in Config.TRAIN_LOCS + Config.TEST_LOCS]:
        sp = os.path.join(sd_path, loc, "0.png")
        if os.path.exists(sp):
            with torch.no_grad():
                f, _ = model(tr(Image.open(sp).convert('RGB')).unsqueeze(0).to(dev))
            sf.append(f.cpu())
            sl.append(tds.location_to_idx[loc] if loc in tds.location_to_idx else -1 - len(gn))
            gn.append(loc)
    sf = torch.cat(sf); sl = torch.tensor(sl)
    print(f"  Gallery:{len(sf)} | Queries:{len(all_drone_feats)} (TTA x{n_augs})")
    return compute_metrics(all_drone_feats, sf, all_labels, sl)
...
class WarmupCosineScheduler:
    ...
    def step(self, e):
        if e < self.we:
            lr = self.blr * (e + 1) / self.we
        else:
            lr = self.mlr + 0.5 * (self.blr - self.mlr) * (1 + math.cos(math.pi * (e - self.we) / (self.te - self.we)))
        for pg in self.opt.param_groups: pg['lr'] = lr
        return lr


def swa_cyclical_lr(opt, epoch, swa_start, swa_lr, cycle_len=5):
    """Cyclical LR for SWA phase"""
    t = ((epoch - swa_start) % cycle_len) / cycle_len
    lr = swa_lr * (1 + math.cos(math.pi * t)) / 2
    lr = max(lr, 1e-6)
...
def main():
    ...
    if args.test: Config.NUM_EPOCHS = 5; Config.NUM_WORKERS = 0; Config.BATCH_SIZE = 8; Config.P = 2; Config.SWA_START = 3

    Config.K = max(2, Config.BATCH_SIZE // Config.P)
    print("=" * 60 + "\nEXP16: GeoSWA — Stochastic Weight Averaging + TTA\n" + "=" * 60)
    ...
    try: teacher = DINOv2Teacher(2).to(dev)
    except: teacher = None
    ...
    for ep in range(Config.NUM_EPOCHS):
        # Phase 1: Normal training with cosine LR
        # Phase 2: SWA with cyclical LR
        if ep < Config.SWA_START:
            lr = sched.step(ep)
        else:
            lr = swa_cyclical_lr(opt, ep, Config.SWA_START, Config.SWA_LR)

        print(f"\n{'=' * 40}\nEp {ep + 1}/{Config.NUM_EPOCHS} LR:{lr:.6f} {'[SWA]' if ep >= Config.SWA_START else ''}\n{'=' * 40}")
        al, ld = train_one_epoch(model, teacher, trl, crit, opt, scaler, dev, ep)
        print(f"Ep {ep + 1} Loss:{al:.4f}")
        [print(f"  {k}:{v:.4f}") for k, v in ld.items()]
        ...
        if (ep + 1) % 5 == 0 or ep == Config.NUM_EPOCHS - 1:
            ...
            if r['R@1'] > best:
                best = r['R@1']
                torch.save({'epoch': ep, 'model': model.state_dict(), 'r1': r['R@1'], 'ap': ap},
                           os.path.join(Config.OUTPUT_DIR, 'best_model_exp16.pth'))
                print("  ★ Best!")

    # === FINAL: Apply SWA + Update BN + Evaluate with TTA ===
    print("\n" + "=" * 60 + "\nFinal: Applying SWA + BN update + TTA evaluation\n" + "=" * 60)
    swa.update_bn(trl, dev)

    # Evaluate SWA model (standard)
    r_swa, ap_swa = evaluate(model, ted, dev)
    print(f"SWA R@1:{r_swa['R@1']:.2f}% R@5:{r_swa['R@5']:.2f}% mAP:{ap_swa:.2f}%")

    # Evaluate SWA model with TTA
    r_tta, ap_tta = evaluate_with_tta(model, ted, dev, Config.TTA_AUGMENTS)
    print(f"SWA+TTA R@1:{r_tta['R@1']:.2f}% R@5:{r_tta['R@5']:.2f}% mAP:{ap_tta:.2f}%")

    final_best = max(best, r_swa['R@1'], r_tta['R@1'])
    if r_tta['R@1'] > best:
        torch.save({'model': model.state_dict(), 'r1': r_tta['R@1'], 'ap': ap_tta, 'swa': True, 'tta': True},
                   os.path.join(Config.OUTPUT_DIR, 'best_model_exp16_swa_tta.pth'))
        print("  ★ SWA+TTA is new best!")

    print(f"\n{'=' * 60}\nDone! Best R@1: {final_best:.2f}%")

    print(f"  Standard:  {best:.2f}%")
    print(f"  SWA:       {r_swa['R@1']:.2f}%")
    print(f"  SWA+TTA:   {r_tta['R@1']:.2f}%")
    print("=" * 60)
